    ...
    pub fn load_schema_with_name(&self, name: &str, schema: Schema) -> Result<(), SchemaError> {
        if schema.name != name {
            return Err(SchemaError::InvalidData(
                format!("Schema name mismatch: expected {name}, got {schema_name}", schema_name = schema.name)
            ));
        }
    ...
    fn validate_mapping_rules(rules: &[MappingRule]) -> Result<(), SchemaError> {
        ...
        for rule in rules {
            let field_name = match rule {
                MappingRule::Map { source_field, .. } | MappingRule::Rename { source_field, .. } => source_field,
        ...
        for mapper in &schema.schema_mappers {
            ...
            let source_schema = schemas.get(&mapper.source_schema_name)
                .ok_or_else(|| SchemaError::NotFound(
                    format!("Source schema not found: {source_schema_name}", source_schema_name = mapper.source_schema_name)
                ))?.clone();
            
            let mut target_schema = schemas.get(&mapper.target_schema_name)
                .ok_or_else(|| SchemaError::NotFound(
                    format!("Target schema not found: {target_schema_name}", target_schema_name = mapper.target_schema_name)
                ))?.clone();
                ...
                match rule {
                    MappingRule::Rename { source_field, target_field } => {
                        ...
                        if !target_schema.fields.contains_key(target_field) {
                            return Err(SchemaError::InvalidField(
                                format!("Target field not found: {target_field}")
                            ));
                        }
                    ...
                    MappingRule::Drop { field } => {
                        if !target_schema.fields.contains_key(field) {
                            return Err(SchemaError::InvalidField(
                                format!("Field to drop not found: {field}")
                            ));
                        }
