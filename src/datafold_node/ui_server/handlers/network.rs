...
use crate::datafold_node::ui_server::types::{ApiSuccessResponse, NetworkInitRequest, ConnectToNodeRequest};
...
pub async fn handle_init_network(
    ...
    let listen_address: SocketAddr = match config.listen_address.parse() {
        Ok(addr) => addr,
        Err(e) => {
            let error = UiError::invalid_request(format!("Invalid listen address: {}", e));
    ...
    match node.init_network(network_config) {
        ...
        Err(e) => {
            let error = UiError::network_error(e.to_string());
