...
impl IntegrationTestSuite {
    ...
    fn run_test_category_auth_enforcement(&mut self) {
        ...
        self.run_test(
            ...
            || {
                let result =
                    authenticator.authenticate_request("/api/v1/query", false, false, None);
                if matches!(result, Err(AuthError::MissingSignature)) {
                    Ok(())
                } else {
                    Err(format!(
                        "Expected missing signature error, got: {:?}",
                        result
                    ))
                }
            },
        ...
        println!(
            "   📊 Metrics: {} total, {} success, {} failed, {} violations",
    ...
    fn run_test_category_performance(&mut self) {
        ...
        self.run_test(
            ...
            || {
                ...
                if avg_duration < 10 {
                    ...
                    Err(format!(
                        "Batch performance failed: avg {}ms >= 10ms",
                        avg_duration
                    ))
                }
            },
        ...
        let metrics = authenticator.get_metrics();
        println!(
            "   📊 Avg processing time: {:.2}ms, Max: {}ms",
    ...
    fn run_test_category_endpoint_protection(&mut self) {
        ...
        self.run_test(
            "Endpoint Protection",
            "Exempted endpoints allow access",
            || {
        ...
        println!(
            "   📊 Protected: {}, Exempted: {}",
            config.protected_endpoints.len(),
            config.exempted_endpoints.len()
        );
        println!();
    }
    ...
    fn run_test_category_cross_platform(&mut self) {
        ...
        self.run_test("Cross-Platform", "Invalid client rejections", || {
            ...
            for (client_name, has_sig, valid_sig, nonce) in &invalid_clients {
                ...
                if result.is_ok() {
                    return Err(format!(
                        "Invalid client {} should have been rejected",
                        client_name
                    ));
                }
            }
        ...
        println!(
            "   📊 Tested {} valid clients and 2 invalid clients",
            test_clients.len()
        );
        println!();
    }
    ...
    fn run_test_category_security(&mut self) {
        ...
        self.run_test("Security Validation", "Replay attack prevention", || {
            ...
            if matches!(second, Err(AuthError::NonceReplay)) {
                ...
                Err(format!(
                    "Replay attack should be prevented, got: {:?}",
                    second
                ))
            }
        });
        ...
        self.run_test(
            ...
            || {
                ...
                if matches!(result, Err(AuthError::InvalidSignature)) {
                    ...
                    Err(format!(
                        "Signature tampering should be detected, got: {:?}",
                        result
                    ))
                }
            },
        ...
        self.run_test("Security Validation", "Security metrics tracking", || {
            ...
            let _invalid = authenticator.authenticate_request(
                "/api/v1/data",
                true,
                false,
                Some("metrics_nonce"),
        ...
        let metrics = authenticator.get_metrics();
        println!(
            "   📊 Security violations: {}, Signature verifications: {}",
    ...
    fn run_test_category_migration(&mut self) {
        ...
        self.run_test(
            ...
            || {
                ...
                let valid_result = authenticator.authenticate_request(
                    "/api/v1/data",
                    true,
                    true,
                    Some("compat_nonce"),
                ...
                if !matches!(valid_result, Ok(AuthResult::Authenticated)) {
                    return Err(
                        "Valid authenticated requests should work in mandatory mode".to_string()
                    );
                }

                // Exempted endpoints should still work
                let exempt_result =
                    authenticator.authenticate_request("/health", false, false, None);
    ...
    fn run_test_category_pbi11_criteria(&mut self) {
        ...
        self.run_test(
            "PBI-11 Criteria",
            "AC1: Endpoint authentication requirement",
            || {
        ...
        self.run_test("PBI-11 Criteria", "AC3: Performance requirements", || {
            let start = Instant::now();
            let perf_result = authenticator.authenticate_request(
                "/api/v1/data",
                true,
                true,
                Some("perf_criteria"),
            ...
            if matches!(perf_result, Ok(AuthResult::Authenticated)) && duration < 10 {
                ...
                Err(format!(
                    "AC3: Performance requirements not met ({}ms)",
                    duration
                ))
            }
        });
        ...
        self.run_test("PBI-11 Criteria", "AC4: Security monitoring", || {
            let metrics_before = authenticator.get_metrics();
            let _violation = authenticator.authenticate_request(
                "/api/v1/data",
                true,
                false,
                Some("monitor_test"),
        ...
        self.run_test(
            ...
            || {
                let cli_result = authenticator.authenticate_request(
                    "/api/v1/data",
                    true,
                    true,
                    Some("cli_compat"),
                );
                let sdk_result = authenticator.authenticate_request(
                    "/api/v1/data",
                    true,
                    true,
                    Some("sdk_compat"),
        ...
        self.run_test("PBI-11 Criteria", "AC6: Replay protection", || {
            let _first = authenticator.authenticate_request(
                ...
                true,
                true,
                Some("replay_criteria"),
            );
            let replay = authenticator.authenticate_request(
                "/api/v1/data",
                true,
                true,
                Some("replay_criteria"),
        ...
        let metrics = authenticator.get_metrics();
        println!(
            "   📊 Final metrics: {} requests, {} violations, {:.2}ms avg",
