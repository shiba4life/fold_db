...
mod tests {
    ...
    fn test_persistence_across_reopen() {
        ...
        let stored_atom: Option<Atom> = db_ops2.get_item(&format!("atom:{}", atom.uuid())).unwrap();
        ...
        assert_eq!(
            stored_aref.unwrap().get_atom_uuid(),
            &atom.uuid().to_string()
        );
    }
    ...
    fn test_unified_schema_operations() {
        ...
        db_ops
            .store_schema_state("TestSchema", SchemaState::Approved)
            .unwrap();
        assert_eq!(
    ...
    fn test_unified_transform_operations() {
        ...
        let transform = Transform::new("return x + 1".to_string(), "test.output".to_string());

        // Store and retrieve transform
        db_ops
            .store_transform("test_transform", &transform)
            .unwrap();
    ...
    fn test_unified_orchestrator_operations() {
        ...
        db_ops
            .store_orchestrator_state("queue_state", &state)
            .unwrap();

        let retrieved: Option<serde_json::Value> =
            db_ops.get_orchestrator_state("queue_state").unwrap();
        assert!(retrieved.is_some());
    ...
    fn test_unified_operations_integration() {
        ...
        db_ops.set_node_id(node_id).unwrap();
        db_ops
            .set_schema_permissions(node_id, &["TestSchema".to_string()])
            .unwrap();
        ...
        db_ops
            .store_orchestrator_state("active_state", &orch_state)
            .unwrap();

        // 6. Verify everything is accessible through unified interface
        assert_eq!(db_ops.get_node_id().unwrap(), node_id);
        assert_eq!(
            db_ops.get_schema_permissions(node_id).unwrap(),
            vec!["TestSchema".to_string()]
        );
        assert_eq!(
            db_ops.get_schema_state("TestSchema").unwrap(),
            Some(SchemaState::Approved)
        ...
        let retrieved_atom: Option<Atom> =
            db_ops.get_item(&format!("atom:{}", atom.uuid())).unwrap();
        ...
        let retrieved_orch_state: Option<serde_json::Value> =
            db_ops.get_orchestrator_state("active_state").unwrap();
    ...
    fn test_schema_core_integration_with_unified_db_ops() {
        ...
        let schema_core = SchemaCore::new("test_path", db_ops.clone(), message_bus).unwrap();
        ...
        schema_core
            .set_schema_state("TestSchema", SchemaState::Approved)
            .unwrap();
        ...
        let approved_schemas = schema_core
