...
impl AsyncEncryptionWrapper {
    ...
    pub async fn new(
        ...
        for &context in contexts::all_contexts() {
            ...
            let async_encryptor = AsyncEncryptionAtRest::new(derived_key, config.performance.clone()).await?;
            async_encryptors.insert(context.to_string(), async_encryptor);
        }
        
        let data_cache = Arc::new(RwLock::new(
            LruCache::new(NonZeroUsize::new(config.data_cache_size).unwrap())
        ));
    ...
    pub async fn with_migration_config(
        ...
        let sync_wrapper = EncryptionWrapper::with_migration_config(db_ops, master_keypair, migration_config)?;
        ...
        for &context in contexts::all_contexts() {
            let derived_key = key_manager.derive_key(context, None);
            let async_encryptor = AsyncEncryptionAtRest::new(derived_key, async_config.performance.clone()).await?;
            async_encryptors.insert(context.to_string(), async_encryptor);
        }
        
        let data_cache = Arc::new(RwLock::new(
            LruCache::new(NonZeroUsize::new(async_config.data_cache_size).unwrap())
        ));
    ...
    pub async fn store_encrypted_item_async<T: Serialize + Send + Sync>(
        ...
        let _permit = self.db_operation_semaphore.acquire().await
            .map_err(|_| SchemaError::InvalidData("Failed to acquire database operation permit".to_string()))?;
        ...
        let encryptor = self.async_encryptors.get(context)
            .ok_or_else(|| SchemaError::InvalidData(format!("Unknown encryption context: {}", context)))?;
        
        // Encrypt asynchronously
        let encrypted_data = encryptor.encrypt_async(&serialized).await
        ...
        task::spawn_blocking(move || {
            db.insert(key_bytes, encrypted_bytes)
        }).await
        ...
        metrics.avg_async_time_micros = metrics.total_async_time_micros / (metrics.async_stores + metrics.async_retrieves);
    ...
    pub async fn get_encrypted_item_async<T: DeserializeOwned + Send + Sync>(
        ...
        if self.config.enable_data_cache {
            ...
            if let Some(cached_data) = cache.get_mut(key) {
                ...
                if !cached_data.is_expired(Duration::from_secs(self.config.data_cache_ttl)) {
                    cached_data.touch();
                    let result = serde_json::from_slice(&cached_data.data)
                        .map_err(|e| SchemaError::InvalidData(format!("Failed to deserialize cached item: {}", e)))?;
        ...
        let encrypted_bytes = task::spawn_blocking(move || {
            db.get(key_bytes)
        }).await
        .map_err(|e| SchemaError::InvalidData(format!("Database task failed: {}", e)))?
        ...
        let decrypted_bytes = encryptor.decrypt_async(&encrypted_data).await
            .map_err(|e| SchemaError::InvalidData(format!("Async decryption failed: {}", e)))?;
        
        // Deserialize the result
        let result = serde_json::from_slice(&decrypted_bytes)
            .map_err(|e| SchemaError::InvalidData(format!("Failed to deserialize decrypted item: {}", e)))?;
    ...
    pub async fn store_batch_async<T: Serialize + Send + Sync>(
        ...
        for chunk in items.chunks(self.config.db_batch_size) {
            let futures: Vec<_> = chunk.iter().map(|(key, item)| {
                self.store_encrypted_item_async(key, item, context)
            }).collect();
    ...
    pub async fn get_batch_async<T: DeserializeOwned + Send + Sync>(
        ...
        for chunk in keys.chunks(self.config.db_batch_size) {
            let futures: Vec<_> = chunk.iter().map(|key| {
                let key_clone = key.clone();
                async move {
                    let result = self.get_encrypted_item_async::<T>(&key_clone, context).await?;
    ...
    pub async fn get_performance_metrics(&self) -> CryptoResult<HashMap<String, serde_json::Value>> {
    ...
    pub async fn benchmark_async_vs_sync(&self, test_data_sizes: Vec<usize>, iterations: usize) -> CryptoResult<HashMap<String, serde_json::Value>> {
        let mut results = HashMap::new();
        ...
        for &size in &test_data_sizes {
            ...
            for i in 0..iterations {
                ...
                self.store_encrypted_item_async(&key, &test_data, contexts::ATOM_DATA).await
                    .map_err(|e| CryptoError::InvalidInput(format!("Async store failed: {}", e)))?;
                let _retrieved: Vec<u8> = self.get_encrypted_item_async(&key, contexts::ATOM_DATA).await
            ...
            for i in 0..iterations {
                ...
                self.sync_wrapper.store_encrypted_item(&key, &test_data, contexts::ATOM_DATA)
                    .map_err(|e| CryptoError::InvalidInput(format!("Sync store failed: {}", e)))?;
                let _retrieved: Vec<u8> = self.sync_wrapper.get_encrypted_item(&key, contexts::ATOM_DATA)
            ...
            let performance_improvement = ((async_ops_per_sec - sync_ops_per_sec) / sync_ops_per_sec) * 100.0;
            
            results.insert(format!("size_{}", size), serde_json::json!({
    ...
    pub async fn validate_performance_requirements(&self, max_overhead_percent: f64) -> CryptoResult<bool> {
...
mod tests {
    ...
    async fn create_test_async_wrapper() -> AsyncEncryptionWrapper {
        ...
        AsyncEncryptionWrapper::new(db_ops, &master_keypair, config).await.unwrap()
    }
    ...
    async fn test_async_store_and_retrieve() {
        ...
        wrapper.store_encrypted_item_async(key, test_data, contexts::ATOM_DATA).await.unwrap();
        
        // Retrieve asynchronously
        let retrieved: Vec<u8> = wrapper.get_encrypted_item_async(key, contexts::ATOM_DATA).await.unwrap().unwrap();
    ...
    async fn test_batch_operations() {
        ...
        // Store batch
        wrapper.store_batch_async(test_items.clone(), contexts::ATOM_DATA).await.unwrap();
        
        // Retrieve batch
        let keys: Vec<String> = test_items.iter().map(|(k, _)| k.clone()).collect();
        let retrieved = wrapper.get_batch_async::<Vec<u8>>(keys, contexts::ATOM_DATA).await.unwrap();
    ...
    async fn test_performance_metrics() {
        ...
        wrapper.store_encrypted_item_async(key, test_data, contexts::ATOM_DATA).await.unwrap();
        let _retrieved: Vec<u8> = wrapper.get_encrypted_item_async(key, contexts::ATOM_DATA).await.unwrap().unwrap();
    ...
    async fn test_cache_functionality() {
        ...
        let _retrieved1: Vec<u8> = wrapper.get_encrypted_item_async(key, contexts::ATOM_DATA).await.unwrap().unwrap();
        
        // Second retrieval (should be cache hit)
        let _retrieved2: Vec<u8> = wrapper.get_encrypted_item_async(key, contexts::ATOM_DATA).await.unwrap().unwrap();
    ...
    async fn test_reset_performance_state() {
        ...
        wrapper.store_encrypted_item_async("reset_key", test_data, contexts::ATOM_DATA).await.unwrap();
