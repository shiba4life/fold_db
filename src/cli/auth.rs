...
impl SignatureInput {
    ...
    pub fn to_signature_input_string(&self) -> String {
        let components_str = self.components
            .iter()
            .map(|c| format!("\"{}\"", c))
            .collect::<Vec<_>>()
            .join(" ");
...
impl CliRequestSigner {
    ...
    pub fn sign_request(&self, request: &mut Request) -> CliAuthResult<()> {
        ...
        let signature_bytes = self.keypair.sign_data(canonical_string.as_bytes())
            .map_err(|e| CliAuthError::SignatureGeneration(format!("Failed to sign request: {}", e)))?;
        ...
        request.headers_mut().insert("x-datafold-client-id", self.profile.client_id.parse()
            .map_err(|e| CliAuthError::HttpHeader(format!("Invalid client-id header: {}", e)))?);
        
        if let Some(user_id) = &self.profile.user_id {
            request.headers_mut().insert("x-datafold-user-id", user_id.parse()
                .map_err(|e| CliAuthError::HttpHeader(format!("Invalid user-id header: {}", e)))?);
        }
    ...
    fn generate_signature_components(&self, request: &Request) -> CliAuthResult<Vec<SignatureComponent>> {
        ...
        if let Some(query) = request.url().query() {
            if !query.is_empty() && !components.iter().any(|c| matches!(c, SignatureComponent::Query)) {
    ...
    fn build_canonical_string(&self, request: &Request, signature_input: &SignatureInput) -> CliAuthResult<String> {
        ...
        let params_line = format!("\"@signature-params\": {}", signature_input.to_signature_input_string());
    ...
    fn extract_component_value(&self, request: &Request, component: &SignatureComponent) -> CliAuthResult<String> {
        match component {
            SignatureComponent::Method => {
                Ok(request.method().as_str().to_uppercase())
            }
            SignatureComponent::TargetUri => {
            ...
            SignatureComponent::Authority => {
                request.url().host_str()
                    ...
                    .ok_or_else(|| CliAuthError::InvalidRequest("Missing authority in URL".to_string()))
            }
            SignatureComponent::Scheme => {
                Ok(request.url().scheme().to_string())
            }
            SignatureComponent::Path => {
                Ok(request.url().path().to_string())
            }
            SignatureComponent::Query => {
                Ok(request.url().query().unwrap_or("").to_string())
            }
...
mod tests {
    ...
    fn test_signature_component_display() {
        ...
        assert_eq!(SignatureComponent::Header("content-type".to_string()).to_string(), "content-type");
    }
    ...
    fn test_component_value_extraction() {
        ...
        assert_eq!(
            signer.extract_component_value(&request, &SignatureComponent::Method).unwrap(),
            "POST"
        );
        assert_eq!(
            signer.extract_component_value(&request, &SignatureComponent::Authority).unwrap(),
            "api.example.com"
        );
        assert_eq!(
            signer.extract_component_value(&request, &SignatureComponent::Scheme).unwrap(),
            "https"
        );
        assert_eq!(
            signer.extract_component_value(&request, &SignatureComponent::Path).unwrap(),
            "/test"
        );
        assert_eq!(
            signer.extract_component_value(&request, &SignatureComponent::Query).unwrap(),
            "param=value"
        );
    }
}