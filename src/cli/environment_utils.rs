...
use crate::config::unified_config::{UnifiedConfigError, UnifiedConfigManager};
...
impl EnvironmentManager {
    ...
    pub fn show_environment_info(&self, environment: Option<&str>) -> EnvironmentResult<()> {
        let env = environment
            .map(|s| s.to_string())
            .unwrap_or_else(|| self.current_environment());
        ...
        println!(
            "  Content Digest: {}",
            env_config.signing.include_content_digest
        );
        ...
        println!(
            ...
            env_config.performance.max_concurrent_signs
        );
    ...
    pub fn compare_environments(&self, env1: &str, env2: &str) -> EnvironmentResult<()> {
        ...
        if config1.signing.policy != config2.signing.policy {
            println!(
                "  Signing Policy: {} vs {}",
                config1.signing.policy, config2.signing.policy
            );
        }

        if config1.signing.timeout_ms != config2.signing.timeout_ms {
            println!(
                "  Signing Timeout: {}ms vs {}ms",
                config1.signing.timeout_ms, config2.signing.timeout_ms
            );
        }
        ...
        if config1.verification.strict_timing != config2.verification.strict_timing {
            println!(
                "  Strict Timing: {} vs {}",
                config1.verification.strict_timing, config2.verification.strict_timing
            );
        }
        ...
        if config1.performance.cache_keys != config2.performance.cache_keys {
            println!(
                "  Cache Keys: {} vs {}",
                config1.performance.cache_keys, config2.performance.cache_keys
            );
        }
    ...
    pub fn get_environment_vars(
    ...
    ) -> EnvironmentResult<HashMap<String, String>> {
        let env = environment
            .map(|s| s.to_string())
            .unwrap_or_else(|| self.current_environment());
        ...
        vars.insert(
            "DATAFOLD_SIGNING_POLICY".to_string(),
            env_config.signing.policy.clone(),
        );
        vars.insert(
            "DATAFOLD_SIGNING_TIMEOUT_MS".to_string(),
            env_config.signing.timeout_ms.to_string(),
        );
        vars.insert(
            "DATAFOLD_CONTENT_DIGEST".to_string(),
            env_config.signing.include_content_digest.to_string(),
        );
        vars.insert(
            "DATAFOLD_STRICT_TIMING".to_string(),
            env_config.verification.strict_timing.to_string(),
        );
        vars.insert(
            "DATAFOLD_LOG_LEVEL".to_string(),
            env_config.logging.level.clone(),
        );
        vars.insert(
            "DATAFOLD_CACHE_KEYS".to_string(),
            env_config.performance.cache_keys.to_string(),
        );
