...
use crate::cli::config::{CliConfigError, CliConfigManager, SignatureSettings};
use crate::cli::signing_config::{AutoSigningConfig, EnhancedSigningConfig, SigningMode};
use crate::config::unified_config::{EnvironmentConfig, UnifiedConfigError, UnifiedConfigManager};
...
impl UnifiedCliConfig {
    ...
    pub fn get_signing_config(&self) -> UnifiedIntegrationResult<CliSigningConfig> {
        ...
        let required_components = signing_config
            .required_components
            .iter()
    ...
    pub fn save_cli_config(&mut self) -> UnifiedIntegrationResult<()> {
        self.cli_manager
            .save()
            .map_err(UnifiedIntegrationError::CliConfig)
    }
...
pub mod integration_utils {
    ...
    pub async fn create_authenticated_client(
        ...
        builder
            .build_authenticated(keypair, profile, Some(signing_config))
            .map_err(|e| {
                UnifiedIntegrationError::InvalidSignatureConfig(format!(
    ...
    pub fn display_environment_status(config: &UnifiedCliConfig) -> UnifiedIntegrationResult<()> {
        ...
        println!(
            "  Signature Components: {} required",
            summary.components_count
        );
        println!(
            "  Timestamp: {}",
            if summary.include_timestamp {
                "✅"
            } else {
                "❌"
            }
