...
impl CliConfigManager {
    ...
    pub fn default_config_path() -> CliConfigResult<PathBuf> {
        let home_dir = dirs::home_dir()
            .ok_or_else(|| CliConfigError::InvalidPath("Unable to determine home directory".to_string()))?;
    ...
    pub fn validate(&self) -> CliConfigResult<()> {
        ...
        if let Some(default_name) = &self.config.default_profile {
            if !self.config.profiles.contains_key(default_name) {
                return Err(CliConfigError::Validation(
                    format!("Default profile '{}' does not exist", default_name)
                ));
            }
        }
        ...
        for (name, profile) in &self.config.profiles {
            if profile.client_id.is_empty() {
                return Err(CliConfigError::Validation(
                    format!("Profile '{}' has empty client_id", name)
                ));
            }
            
            if profile.key_id.is_empty() {
                return Err(CliConfigError::Validation(
                    format!("Profile '{}' has empty key_id", name)
                ));
            }
            
            if profile.server_url.is_empty() {
                return Err(CliConfigError::Validation(
                    format!("Profile '{}' has empty server_url", name)
                ));
            }
        }
        ...
        for (name, server) in &self.config.servers {
            if server.url.is_empty() {
                return Err(CliConfigError::Validation(
                    format!("Server '{}' has empty URL", name)
                ));
            }
            
            if server.timeout_secs == 0 {
                return Err(CliConfigError::Validation(
                    format!("Server '{}' has invalid timeout", name)
                ));
            }
        }
    ...
    pub fn set_command_signing_mode(&mut self, command: String, mode: SigningMode) -> CliConfigResult<()> {
        self.config.signing.auto_signing.set_command_mode(command, mode);
        Ok(())
    }
    ...
    pub fn get_signing_status(&self, command: &str) -> bool {
        self.config.signing.auto_signing.is_effective_auto_signing(command)
    }
}
...
mod tests {
    ...
    fn test_profile_management() {
        ...
        manager.add_profile("test".to_string(), profile.clone()).unwrap();
    ...
    fn test_config_validation() {
        ...
        manager.add_profile("invalid".to_string(), invalid_profile).unwrap();
