...
use crate::crypto::ed25519::{PUBLIC_KEY_LENGTH, SIGNATURE_LENGTH, verify_signature};
...
impl CliSignatureVerifier {
    ...
    pub fn add_public_key(&mut self, key_id: String, public_key_bytes: Vec<u8>) -> VerificationResult<()> {
        if public_key_bytes.len() != PUBLIC_KEY_LENGTH {
            return Err(VerificationError::KeyManagement(
                format!("Invalid public key length: expected {}, got {}", PUBLIC_KEY_LENGTH, public_key_bytes.len())
            ));
        }
    ...
    pub async fn verify_response_with_context(
        ...
        step_timings.insert("signature_extraction".to_string(), step_start.elapsed().as_millis() as u64);
        ...
        step_timings.insert("body_extraction".to_string(), step_start.elapsed().as_millis() as u64);
        ...
        self.verify_signature_data(&signature_data, request_context, policy, step_timings, start_time).await
    }
    ...
    pub async fn verify_message_signature(
        ...
        step_timings.insert("key_retrieval".to_string(), step_start.elapsed().as_millis() as u64);

        // Decode signature
        let step_start = Instant::now();
        let signature_bytes = general_purpose::STANDARD.decode(signature_b64)
            .map_err(|e| VerificationError::SignatureFormat(format!("Invalid base64 signature: {}", e)))?;
        
        if signature_bytes.len() != SIGNATURE_LENGTH {
            return Err(VerificationError::SignatureFormat(
                format!("Invalid signature length: expected {}, got {}", SIGNATURE_LENGTH, signature_bytes.len())
            ));
        }
        
        let mut signature_array = [0u8; SIGNATURE_LENGTH];
        signature_array.copy_from_slice(&signature_bytes);
        step_timings.insert("signature_decoding".to_string(), step_start.elapsed().as_millis() as u64);

        // Verify signature
        let step_start = Instant::now();
        let crypto_valid = verify_signature(&public_key_bytes, message, &signature_array).is_ok();
        step_timings.insert("cryptographic_verification".to_string(), step_start.elapsed().as_millis() as u64);
        ...
        Ok(VerificationResultData {
            ...
            error: if crypto_valid { None } else { 
    ...
    /// Extract signature data from response headers
    fn extract_signature_data_from_response(&self, response: &Response) -> VerificationResult<ExtractedSignatureData> {
        let headers = response.headers();

        // Get signature-input header
        let signature_input_header = headers.get("signature-input")
            .ok_or_else(|| VerificationError::MissingComponent("signature-input header".to_string()))?
            .to_str()
            .map_err(|e| VerificationError::SignatureFormat(format!("Invalid signature-input header: {}", e)))?;

        // Get signature header
        let signature_header = headers.get("signature")
            .ok_or_else(|| VerificationError::MissingComponent("signature header".to_string()))?
            .to_str()
            .map_err(|e| VerificationError::SignatureFormat(format!("Invalid signature header: {}", e)))?;

        // Parse signature input (simplified parsing for now)
        let (signature_id, covered_components, parameters) = self.parse_signature_input(signature_input_header)?;
    ...
    fn parse_signature_input(&self, _input: &str) -> VerificationResult<(String, Vec<SignatureComponent>, HashMap<String, String>)> {
    ...
    fn parse_content_digest(&self, digest_header: &str) -> Option<ContentDigest> {
        ...
        if let Some(colon_pos) = digest_header.find(':') {
            let algorithm = digest_header[..colon_pos.min(digest_header.len().saturating_sub(1))].to_string();
    ...
    async fn verify_signature_data(
        ...
        step_timings.insert("format_verification".to_string(), step_start.elapsed().as_millis() as u64);

        let step_start = Instant::now();
        let timestamp_valid = self.verify_timestamp(signature_data, policy)?;
        step_timings.insert("timestamp_verification".to_string(), step_start.elapsed().as_millis() as u64);

        let step_start = Instant::now();
        let nonce_valid = self.verify_nonce(signature_data, policy)?;
        step_timings.insert("nonce_verification".to_string(), step_start.elapsed().as_millis() as u64);

        let step_start = Instant::now();
        let content_digest_valid = self.verify_content_digest(signature_data, request_context.body, policy)?;
        step_timings.insert("content_digest_verification".to_string(), step_start.elapsed().as_millis() as u64);

        let step_start = Instant::now();
        let component_coverage_valid = self.verify_component_coverage(signature_data, policy)?;
        step_timings.insert("component_coverage_verification".to_string(), step_start.elapsed().as_millis() as u64);
        ...
        let checks = VerificationChecks {
            ...
            policy_compliance_valid: format_valid && timestamp_valid && nonce_valid && content_digest_valid && component_coverage_valid,
        };

        let overall_valid = checks.format_valid && checks.cryptographic_valid && checks.timestamp_valid
                          && checks.nonce_valid && checks.content_digest_valid && checks.component_coverage_valid;
        ...
        Ok(VerificationResultData {
            ...
            error: if overall_valid { None } else {
                Some(VerificationErrorInfo {
    ...
    fn verify_signature_format(&self, signature_data: &ExtractedSignatureData) -> VerificationResult<bool> {
        // Verify base64 signature format
        if general_purpose::STANDARD.decode(&signature_data.signature).is_err() {
            return Ok(false);
        }
    ...
    fn verify_timestamp(&self, signature_data: &ExtractedSignatureData, policy: &VerificationPolicy) -> VerificationResult<bool> {
    ...
    /// Verify nonce format
    fn verify_nonce(&self, signature_data: &ExtractedSignatureData, policy: &VerificationPolicy) -> VerificationResult<bool> {
        if !policy.verify_nonce {
            return Ok(true);
        }

        // Check if nonce is present and properly formatted
        signature_data.parameters.get("nonce")
            .map(|nonce| !nonce.is_empty())
            .unwrap_or(false)
            .then_some(true)
            .ok_or(VerificationError::PolicyViolation("Missing or invalid nonce".to_string()))
            .map(|_| true)
    }

    /// Verify content digest
    fn verify_content_digest(&self, signature_data: &ExtractedSignatureData, body: Option<&[u8]>, policy: &VerificationPolicy) -> VerificationResult<bool> {
        ...
        if let Some(digest) = &signature_data.content_digest {
            if let Some(body_bytes) = body {
                ...
                let calculated_digest = match digest.algorithm.as_str() {
                    ...
                    _ => return Err(VerificationError::Configuration(format!("Unsupported digest algorithm: {}", digest.algorithm))),
                };
    ...
    fn verify_component_coverage(&self, signature_data: &ExtractedSignatureData, policy: &VerificationPolicy) -> VerificationResult<bool> {
        for required_component in &policy.required_components {
            if !signature_data.covered_components.contains(required_component) {
                return Ok(false);
            }
        }
        Ok(true)
    ...
    fn get_public_key(&self, key_id: &str) -> VerificationResult<[u8; PUBLIC_KEY_LENGTH]> {
        let key_bytes = self.config.public_keys.get(key_id)
            .ok_or_else(|| VerificationError::KeyManagement(format!("Unknown key ID: {}", key_id)))?;

        if key_bytes.len() != PUBLIC_KEY_LENGTH {
            return Err(VerificationError::KeyManagement(
                format!("Invalid key length for {}: expected {}, got {}", key_id, PUBLIC_KEY_LENGTH, key_bytes.len())
            ));
        }
    ...
    fn build_diagnostics(&self, signature_data: &ExtractedSignatureData, policy: &VerificationPolicy, body: Option<&[u8]>) -> VerificationDiagnostics {
        let signature_analysis = SignatureAnalysis {
            algorithm: signature_data.parameters.get("alg").cloned().unwrap_or_default(),
            key_id: signature_data.parameters.get("keyid").cloned().unwrap_or_default(),
            created: signature_data.parameters.get("created").and_then(|s| s.parse().ok()),
            age_seconds: signature_data.parameters.get("created")
                .and_then(|s| s.parse::<i64>().ok())
                .map(|created| (Utc::now().timestamp() - created) as u64),
            nonce: signature_data.parameters.get("nonce").cloned(),
            covered_components: signature_data.covered_components.iter().map(|c| c.to_string()).collect(),
        };

        let content_analysis = ContentAnalysis {
            has_content_digest: signature_data.content_digest.is_some(),
            digest_algorithm: signature_data.content_digest.as_ref().map(|d| d.algorithm.clone()),
            content_size: body.map(|b| b.len()).unwrap_or(0),
            content_type: None, // Would extract from headers in real implementation
        };

        let missing_required_components = policy.required_components.iter()
...
impl SignatureInspector {
    ...
    pub fn inspect_signature_format(&self, headers: &HashMap<String, String>) -> SignatureFormatAnalysis {
        let mut issues = vec![];
    ...
    pub fn generate_diagnostic_report(&self, result: &VerificationResultData) -> String {
        ...
        report.push_str(&format!("Components: {}\n\n", result.diagnostics.signature_analysis.covered_components.join(", ")));
...
mod tests {
    ...
    fn test_signature_inspector() {
        ...
        headers.insert("signature-input".to_string(), "sig1=(\"@method\" \"@target-uri\");alg=\"ed25519\"".to_string());
