...
impl TransformUtils {
    ...
    pub fn resolve_field_value(
        ...
        match field {
            FieldVariant::Range(_) => {
                ...
                match db_ops
                    .get_item::<crate::atom::AtomRefRange>(&format!("ref:{}", range_aref_uuid))
                {
                    Ok(Some(range_aref)) => {
                        info!(
                            "✅ Found AtomRefRange with {} entries",
                            range_aref.atom_uuids.len()
                        );
        ...
        error!(
            "🚨 CRITICAL BUG: Reading STATIC schema ref_atom_uuid: {}",
            ref_atom_uuid
        );
        ...
        error!(
            "🔍 DIAGNOSTIC: Checking dynamic AtomRef UUID: {}",
            dynamic_aref_uuid
        );
        ...
        match db_ops.get_item::<crate::atom::AtomRef>(&format!("ref:{}", dynamic_aref_uuid)) {
            Ok(Some(dynamic_aref)) => {
                ...
                error!(
                    "🔍 DIAGNOSTIC: Dynamic AtomRef points to atom: {}",
                    dynamic_atom_uuid
                );
                error!(
                    "🚨 MISMATCH DETECTED: Static schema: {} vs Dynamic AtomRef: {}",
                    ref_atom_uuid, dynamic_atom_uuid
                );
                ...
                info!(
                    "✅ Fixed query using dynamic AtomRef - content: {}",
                    content
                );
                return Ok(content);
            }
    ...
    fn extract_ref_atom_uuid(
        field: &FieldVariant,
        field_name: &str,
    ) -> Result<String, SchemaError> {
        let ref_atom_uuid = field
            .ref_atom_uuid()
            .ok_or_else(|| {
...
mod tests {
    ...
    fn test_default_value_helper() {
        assert_eq!(
            TransformUtils::get_default_value_for_field("input1"),
            JsonValue::Number(serde_json::Number::from(42))
        );
        assert_eq!(
