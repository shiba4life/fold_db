...
impl EventHandler for AuditLogHandler {
    async fn handle_event(&self, event: &SecurityEvent) -> EventHandlerResult {
        ...
        let log_entry = if self.structured {
            ...
            format!(
                ...
                match &base_event.result {
                    ...
                    crate::events::event_types::OperationResult::Failure { error_type, .. } =>
                        format!("FAILED: {}", error_type),
                    crate::events::event_types::OperationResult::Cancelled =>
                        "CANCELLED".to_string(),
                    crate::events::event_types::OperationResult::InProgress =>
        ...
        match OpenOptions::new()
            ...
            .await
        {
            Ok(mut file) => match file.write_all(log_entry.as_bytes()).await {
                Ok(_) => {
                    ...
                    metadata.insert(
                        ...
                        serde_json::Value::Bool(self.structured),
                    );
    ...
    async fn handle_event(&self, event: &SecurityEvent) -> EventHandlerResult {
        ...
        *metrics.entry(result_key.to_string()).or_insert(0.0) += 1.0;
        ...
        result_metadata.insert(
            ...
            serde_json::Value::Number(metrics.len().into()),
        );
...
impl EventHandler for SecurityAlertHandler {
    async fn handle_event(&self, event: &SecurityEvent) -> EventHandlerResult {
        ...
        if !event.should_alert(self.min_severity) {
            return EventHandlerResult {
                ...
                metadata: {
                    ...
                    metadata.insert(
                        "reason".to_string(),
                        serde_json::Value::String("Below severity threshold".to_string()),
                    );
                    metadata
                },
            };
        ...
        let alert_message = format!(
            ...
            base_event.component,
            match &base_event.result {
                crate::events::event_types::OperationResult::Success =>
                    "completed successfully".to_string(),
                crate::events::event_types::OperationResult::Failure { error_message, .. } =>
                    format!("failed: {}", error_message),
                crate::events::event_types::OperationResult::Cancelled =>
                    "was cancelled".to_string(),
                crate::events::event_types::OperationResult::InProgress =>
                    "is in progress".to_string(),
        ...
        for destination in &self.alert_destinations {
            match destination {
                ...
                AlertDestination::Webhook { url, headers } => {
                    if let Err(e) = self
                        .send_webhook_alert(url, headers, &alert_message, event)
                        .await
                    {
                        success = false;
        ...
        metadata.insert(
            ...
            serde_json::Value::Number(self.alert_destinations.len().into()),
        );
        metadata.insert(
            "alert_message".to_string(),
            serde_json::Value::String(alert_message),
        );
