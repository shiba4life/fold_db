...
use super::event_types::{SecurityEvent, SecurityEventCategory, PlatformSource};
...
impl CorrelationGroup {
    ...
    pub fn new(primary_key: String, strategy: CorrelationStrategy, initial_event: SecurityEvent) -> Self {
    ...
    pub fn summary(&self) -> CorrelationSummary {
        CorrelationSummary {
            ...
            severity_levels: self.events.iter()
                .map(|e| e.severity())
                .collect::<HashSet<_>>()
                .len(),
        }
...
impl CorrelationManager {
    ...
    pub async fn get_correlated_events(&self, event_id: Uuid) -> Vec<SecurityEvent> {
        if let Some(group_id) = self.event_to_group.get(&event_id) {
            if let Some(group) = self.correlation_groups.values().find(|g| g.group_id == *group_id) {
    ...
    pub async fn get_correlation_group(&self, group_id: Uuid) -> Option<&CorrelationGroup> {
        self.correlation_groups.values().find(|g| g.group_id == group_id)
    }
    ...
    pub async fn get_cross_platform_correlations(&self) -> Vec<&CorrelationGroup> {
        self.correlation_groups.values()
    ...
    pub async fn get_statistics(&self) -> CorrelationStatistics {
        ...
        if stats.total_correlations > 0 {
            let total_events: usize = self.correlation_groups.values()
                .map(|g| g.events.len())
                .sum();
            stats.avg_events_per_correlation = total_events as f64 / stats.total_correlations as f64;
        }
    ...
    pub async fn search_correlations(&self, criteria: &CorrelationSearchCriteria) -> Vec<&CorrelationGroup> {
        self.correlation_groups.values()
    ...
    async fn correlate_event(&mut self, event: &SecurityEvent, strategy: CorrelationStrategy) -> bool {
        let base_event = event.base_event();
        
        let correlation_key = match strategy {
            CorrelationStrategy::TraceId => {
                base_event.trace_id.clone()
            }
            CorrelationStrategy::CorrelationId => {
                base_event.correlation_id.map(|id| id.to_string())
            }
            CorrelationStrategy::SessionId => {
                base_event.session_id.clone()
            }
            CorrelationStrategy::Actor => {
                base_event.actor.clone()
            }
        ...
        if let Some(key) = correlation_key {
            ...
            if let Some(group) = self.correlation_groups.get_mut(&group_key) {
                ...
                self.event_to_group.insert(base_event.event_id, group.group_id);
                ...
                *self.statistics.correlations_by_strategy.entry(strategy).or_insert(0) += 1;
    ...
    async fn auto_discover_correlations(&mut self, event: &SecurityEvent) {
        ...
        if self.config.strategies.contains(&CorrelationStrategy::TemporalProximity) {
            self.discover_temporal_correlations(event).await;
        }
        
        // Look for operation sequence patterns
        if self.config.strategies.contains(&CorrelationStrategy::OperationSequence) {
            self.discover_operation_sequences(event).await;
        }
        
        // Look for component interaction patterns
        if self.config.strategies.contains(&CorrelationStrategy::ComponentInteraction) {
    ...
    async fn discover_temporal_correlations(&mut self, event: &SecurityEvent) {
        ...
        let candidates: Vec<&SecurityEvent> = self.recent_events.iter()
        ...
        if !candidates.is_empty() {
            let correlation_key = format!("temporal_{}_{}", 
            ...
            if let Some(group) = self.correlation_groups.get_mut(&correlation_key) {
                ...
                for candidate in candidates {
                    ...
                    self.event_to_group.insert(candidate.base_event().event_id, group_id);
                }
                ...
                self.statistics.total_correlations += 1;
                *self.statistics.correlations_by_strategy.entry(CorrelationStrategy::TemporalProximity).or_insert(0) += 1;
    ...
    async fn cleanup_old_data(&mut self) {
        let cutoff_time = Utc::now() - ChronoDuration::from_std(self.config.correlation_window).unwrap();
...
mod tests {
    use super::*;
    use crate::events::event_types::{CreateVerificationEvent, VerificationEvent, SecurityEvent, SecurityEventCategory, EventSeverity, PlatformSource};
    ...
    async fn test_trace_id_correlation() {
        ...
        manager.add_event(&SecurityEvent::Generic(event1.clone())).await;
        manager.add_event(&SecurityEvent::Generic(event2.clone())).await;
    ...
    fn test_correlation_group_operations() {
        ...
        let mut group = CorrelationGroup::new(
            "test-key".to_string(),
            CorrelationStrategy::TraceId,
            event1,
        );
