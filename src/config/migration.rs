...
impl ConfigMigrationManager {
    ...
    pub async fn migrate_cli_config(&self) -> ConfigResult<MigrationResult> {
        ...
        for legacy_path in legacy_paths {
            if legacy_path.exists() {
                return self.migrate_config_file(
                    &legacy_path,
    ...
    pub async fn migrate_logging_config(&self) -> ConfigResult<MigrationResult> {
        ...
        if legacy_path.exists() {
            self.migrate_logging_config_file(&legacy_path, &target_path).await
        } else {
            Ok(MigrationResult {
                success: true,
    ...
    pub async fn migrate_unified_config(&self) -> ConfigResult<MigrationResult> {
        ...
        if legacy_path.exists() {
            self.migrate_unified_config_file(&legacy_path, &target_path).await
        } else {
            Ok(MigrationResult {
                success: true,
                source_path: legacy_path,
    ...
    pub async fn migrate_config_file(
        ...
        if let Some(parent) = target_path.parent() {
            fs::create_dir_all(parent).await
                .map_err(|e| ConfigError::platform(format!("Failed to create target directory: {}", e)))?;
        }

        match strategy {
            MigrationStrategy::DirectCopy => {
                fs::copy(source_path, target_path).await
                    .map_err(|e| ConfigError::platform(format!("Failed to copy configuration: {}", e)))?;
                result.success = true;
            ...
            MigrationStrategy::Transform => {
                let content = fs::read_to_string(source_path).await
                    .map_err(|e| ConfigError::platform(format!("Failed to read source configuration: {}", e)))?;
                
                let transformed = self.transform_config_content(&content, source_path, target_path).await?;
                
                fs::write(target_path, transformed).await
                    .map_err(|e| ConfigError::platform(format!("Failed to write transformed configuration: {}", e)))?;
            ...
            MigrationStrategy::Custom(transform_fn) => {
                ...
                fs::write(target_path, transformed).await
                    .map_err(|e| ConfigError::platform(format!("Failed to write custom transformed configuration: {}", e)))?;
    ...
    async fn transform_config_content(
        ...
        match source_extension {
            ...
            _ => {
                Err(ConfigError::validation(format!("Unsupported configuration format: {}", source_extension)))
            }
        }
    }
    ...
    fn parse_legacy_config(&self, content: &str, source_path: &Path) -> ConfigResult<Config> {
        ...
        match extension {
            ...
            _ => {
                Err(ConfigError::validation(format!("Unsupported format: {}", extension)))
            }
        }
    }
    ...
    async fn migrate_logging_config_file(
        ...
        if updated_config.outputs.file.enabled {
            ...
            updated_config.outputs.file.path = logs_dir.join(log_file_name).to_string_lossy().to_string();
        }
        ...
        if let Some(ref path) = updated_config.outputs.structured.path {
            ...
            updated_config.outputs.structured.path = Some(logs_dir.join(log_file_name).to_string_lossy().to_string());
        }
        ...
        let updated_content = toml::to_string_pretty(&updated_config)
            .map_err(|e| ConfigError::platform(format!("Failed to serialize logging config: {}", e)))?;

        fs::write(target_path, updated_content).await
    ...
    async fn migrate_unified_config_file(
        ...
        let unified_config: crate::config::unified_config::UnifiedConfig = serde_json::from_str(&content)
            .map_err(|e| ConfigError::validation(format!("Invalid unified config: {}", e)))?;

        // Convert to enhanced configuration format
        let enhanced_config = self.convert_unified_to_enhanced(unified_config)?;
        ...
        let enhanced_content = toml::to_string_pretty(&enhanced_config)
            .map_err(|e| ConfigError::platform(format!("Failed to serialize enhanced config: {}", e)))?;

        fs::write(target_path, enhanced_content).await
            .map_err(|e| ConfigError::platform(format!("Failed to write enhanced config: {}", e)))?;
    ...
    fn convert_unified_to_enhanced(
        ...
        for (env_name, env_config) in unified.environments {
            ...
            env_section.insert("signing".to_string(), 
                ConfigValue::object({
                    let mut signing = HashMap::new();
                    signing.insert("policy".to_string(), ConfigValue::string(env_config.signing.policy));
                    signing.insert("timeout_ms".to_string(), ConfigValue::integer(env_config.signing.timeout_ms as i64));
                    signing.insert("include_content_digest".to_string(), ConfigValue::boolean(env_config.signing.include_content_digest));
                    signing
                })
            );
            
            // Add verification configuration
            env_section.insert("verification".to_string(),
                ConfigValue::object({
                    let mut verification = HashMap::new();
                    verification.insert("strict_timing".to_string(), ConfigValue::boolean(env_config.verification.strict_timing));
                    verification.insert("allow_clock_skew_seconds".to_string(), ConfigValue::integer(env_config.verification.allow_clock_skew_seconds as i64));
                    verification
                })
            );
            
            enhanced.base.set_section(env_name, ConfigValue::object(env_section));
        }
