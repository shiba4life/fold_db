...
use crate::config::platform::{PlatformConfigPaths, create_platform_resolver, get_platform_info};
...
impl TomlConfigProvider {
    ...
    async fn migrate_legacy_config(&self) -> ConfigResult<Option<Config>> {
        ...
        if legacy_path.exists() {
            ...
            config.metadata.insert("migrated_from".to_string(), 
                ConfigValue::string(legacy_path.to_string_lossy()));
            config.metadata.insert("migration_timestamp".to_string(),
                ConfigValue::string(chrono::Utc::now().to_rfc3339()));
            
            // Store the original JSON data for manual migration
            config.metadata.insert("legacy_data".to_string(),
...
impl ConfigurationProvider for TomlConfigProvider {
    ...
    async fn save(&self, config: &Config) -> ConfigResult<()> {
        ...
        tokio::fs::write(&temp_path, toml_content).await
            .map_err(ConfigError::from)?;
    ...
    async fn validate(&self, config: &Config) -> ConfigResult<()> {
        ...
        if config.version.is_empty() {
            return Err(ConfigError::validation("Configuration version cannot be empty"));
        }
...
impl Config {
    ...
    pub fn new() -> Self {
        ...
        metadata.insert("platform".to_string(), ConfigValue::string(platform_info.name));
        metadata.insert("architecture".to_string(), ConfigValue::string(platform_info.arch));
...
impl PlatformRequirements {
    ...
    pub fn is_compatible(&self, platform: &str) -> bool {
        self.supported_platforms.iter().any(|p| p == platform || p == "*")
    }
}
...
mod tests {
    ...
    async fn test_toml_provider_basic_operations() {
        ...
        assert_eq!(loaded_config.get_section("test").unwrap().as_string().unwrap(), "value");
    }
    ...
    async fn test_configuration_manager() {
        ...
        assert_eq!(reloaded_config.get_section("app").unwrap().as_string().unwrap(), "test_app");
    }
    ...
    fn test_config_value_operations() {
        ...
        config.set_section("logging".to_string(), ConfigValue::object({
    ...
    fn test_config_merge() {
        ...
        assert_eq!(config1.get_section("section1").unwrap().as_string().unwrap(), "new_value1");
        assert_eq!(config1.get_section("section2").unwrap().as_string().unwrap(), "value2");
    }
}