...
use crate::config::{
    ...
    migration::{ConfigMigrationManager, MigrationResult, MigrationStrategy},
...
use super::{
    ...
    mocks::{MockKeystore, MockPerformanceMonitor, MockPlatformPaths},
    utils::*,
};
...
impl IntegrationTestResults {
    pub fn print_summary(&self) {
        ...
        println!(
            "   Data Integrity: {:.1}%",
            self.data_integrity_score * 100.0
        );
...
mod cli_integration_tests {
    ...
    async fn test_cli_config_migration() {
        ...
        tokio::fs::write(&legacy_file, legacy_content)
            .await
            .unwrap();
        ...
        let migration_result = migration_manager
            .migrate_config_file(&legacy_file, &target_file, MigrationStrategy::Transform)
            .await;
        ...
        match migration_result {
                ...
                if result.success {
                    ...
                    if let Ok(auth_profile) = migrated_config.get_value("cli.auth.profile") {
                        ...
                        results
                            ...
                            .push("Auth profile not migrated correctly".to_string());
                    ...
                    if let Ok(server_url) = migrated_config.get_value("cli.server.url") {
                        ...
                        results
                            .compatibility_issues
                            .push("Server URL not migrated correctly".to_string());
                        results.data_integrity_score -= 0.2;
                    }
                    ...
                    if migrated_config.get_value("cli.auth.api_key").is_ok() {
                        results
                            .compatibility_issues
                            .push("Sensitive API key migrated to plaintext config".to_string());
                        results
                            .recommendations
                            .push("Move API keys to secure keystore".to_string());
            ...
            Err(e) => {
                ...
                results
                    .compatibility_issues
                    .push(format!("Migration failed: {}", e));
            }
        }
        ...
        if migration_time > Duration::from_secs(5) {
            results
                .compatibility_issues
                .push("CLI migration too slow".to_string());
        }
    ...
    async fn test_cli_config_compatibility() {
        ...
        server_section.insert(
            "url".to_string(),
            ConfigValue::string("https://api.datafold.com"),
        );
        ...
        config.set_section(
            ...
            ConfigValue::object(server_section),
        );
        ...
        config.set_section(
            ...
            ConfigValue::object(output_section),
        );
        ...
        for section_path in cli_sections {
            if loaded_config.get_value(section_path).is_err() {
                results
                    .compatibility_issues
                    .push(format!("CLI section missing: {}", section_path));
        ...
        updated_server.insert(
            "url".to_string(),
            ConfigValue::string("https://staging.datafold.com"),
        );
        ...
        updated_config.set_section(
            "cli.server".to_string(),
            ConfigValue::object(updated_server),
        );
        ...
        let new_url = final_config
            .get_value("cli.server.url")
            .unwrap()
            .as_string()
            .unwrap();
        ...
        if update_time > Duration::from_millis(100) {
            results
                .compatibility_issues
                .push("CLI config updates too slow".to_string());
        }
...
mod node_integration_tests {
    ...
    async fn test_node_config_integration() {
        ...
        crypto_section.insert(
            "encryption_algorithm".to_string(),
            ConfigValue::string("AES-256-GCM"),
        );
        crypto_section.insert(
            "key_derivation".to_string(),
            ConfigValue::string("Argon2id"),
        );
        crypto_section.insert(
            "enable_encryption_at_rest".to_string(),
            ConfigValue::boolean(true),
        );
        config.set_section(
            "node.crypto".to_string(),
            ConfigValue::object(crypto_section),
        );
        ...
        logging_section.insert(
            "enable_structured_logging".to_string(),
            ConfigValue::boolean(true),
        );
        config.set_section(
            "node.logging".to_string(),
            ConfigValue::object(logging_section),
        );
        ...
        for (section_path, expected_value) in node_sections {
            match loaded_config.get_value(section_path) {
                ...
                Err(_) => {
                    results
                        .compatibility_issues
                        .push(format!("Node section missing: {}", section_path));
                    results.data_integrity_score -= 0.2;
                }
            }
        }
        ...
        if reload_time > MAX_HOT_RELOAD_TIME {
            results
                .compatibility_issues
                .push("Node config hot reload too slow".to_string());
        }

        // Verify configuration consistency after reload
        assert_eq!(
            reloaded_config
                .get_value("node.server.port")
                .unwrap()
                .as_integer()
                .unwrap(),
            8080
        ...
        if access_time > MAX_LOAD_TIME {
            results
                .compatibility_issues
                .push("Node config access too slow".to_string());
        }
    ...
    async fn test_node_crypto_config_integration() {
        ...
        config.set_section(
            "crypto.encryption".to_string(),
            ConfigValue::object(encryption_section),
        );
        ...
        config.set_section(
            "crypto.key_rotation".to_string(),
            ConfigValue::object(rotation_section),
        );

        // Signing configuration
        let mut signing_section = HashMap::new();
        signing_section.insert("algorithm".to_string(), ConfigValue::string("Ed25519"));
        signing_section.insert(
            "enable_verification".to_string(),
            ConfigValue::boolean(true),
        );
        config.set_section(
            "crypto.signing".to_string(),
            ConfigValue::object(signing_section),
        );
        ...
        for _i in 0..10 {
            ...
            if access_time > Duration::from_millis(5) {
                results
                    .compatibility_issues
                    .push("Crypto config access too slow for node operations".to_string());
                break;
            }
        }
        ...
        for setting in crypto_settings {
            if loaded_config.get_value(setting).is_err() {
                results
                    .compatibility_issues
                    .push(format!("Crypto setting missing: {}", setting));
                results.data_integrity_score -= 0.2;
            }
        }
        ...
        updated_config.set_section(
            "crypto.key_rotation".to_string(),
            ConfigValue::object(updated_rotation),
        );
        ...
        if update_time > Duration::from_millis(50) {
            results
                .compatibility_issues
                .push("Crypto config updates too slow".to_string());
        }
...
mod logging_integration_tests {
    ...
    async fn test_logging_config_alignment() {
        ...
        config.set_section(
            "logging.global".to_string(),
            ConfigValue::object(global_logging),
        );
        ...
        file_output.insert(
            "path".to_string(),
            ConfigValue::string("/var/log/datafold/datafold.log"),
        );
        ...
        config.set_section(
            "logging.outputs.file".to_string(),
            ConfigValue::object(file_output),
        );
        ...
        config.set_section(
            "logging.outputs.console".to_string(),
            ConfigValue::object(console_output),
        );
        ...
        config.set_section(
            "logging.outputs.structured".to_string(),
            ConfigValue::object(structured_output),
        );
        ...
        for section in logging_sections {
            if loaded_config.get_value(section).is_err() {
                results
                    .compatibility_issues
                    .push(format!("Logging section missing: {}", section));
                results.data_integrity_score -= 0.1;
            }
        }

        // Test logging configuration validation
        let global_level = loaded_config
            .get_value("logging.global.level")
            .unwrap()
            .as_string()
            .unwrap();
        ...
        if !valid_levels.contains(&global_level.as_str()) {
            results
                .compatibility_issues
                .push(format!("Invalid logging level: {}", global_level));
            results.data_integrity_score -= 0.2;
        }
        ...
        updated_config.set_section(
            "logging.global".to_string(),
            ConfigValue::object(updated_global),
        );
        ...
        if update_time > Duration::from_millis(20) {
            results
                .compatibility_issues
                .push("Logging config updates too slow for runtime changes".to_string());
        }

        // Verify the change was applied
        let final_config = manager.get().await.unwrap();
        let new_level = final_config
            .get_value("logging.global.level")
            .unwrap()
            .as_string()
            .unwrap();
        ...
        if access_time > Duration::from_millis(5) {
            results
                .compatibility_issues
                .push("Logging config access too slow".to_string());
        }
...
mod migration_integration_tests {
    ...
    async fn test_comprehensive_system_migration() {
        ...
        if total_migration_time > Duration::from_secs(30) {
            results
                .compatibility_issues
                .push("Total migration time too long".to_string());
        }
        ...
        if !results.migration_successful {
            results
                .recommendations
                .push("Review failed migrations and implement fallback strategies".to_string());
        }

        if total_migration_time > Duration::from_secs(10) {
            results
                .recommendations
                .push("Optimize migration performance for large deployments".to_string());
        }

        results
            .recommendations
            .push("Implement migration rollback capability".to_string());
        results
            .recommendations
            .push("Add migration validation and verification".to_string());
...
async fn create_multiple_legacy_configs(temp_dir: &std::path::Path) -> Vec<PathBuf> {
    ...
    tokio::fs::write(
        &cli_path,
        serde_json::to_string_pretty(&cli_config).unwrap(),
    )
    .await
    .unwrap();
    ...
    tokio::fs::write(
        &logging_path,
        serde_json::to_string_pretty(&logging_config).unwrap(),
    )
    .await
    .unwrap();
...
pub async fn run_integration_test_suite() -> Vec<IntegrationTestResults> {
    ...
    let successful_migrations = all_results
        .iter()
        .filter(|r| r.migration_successful)
        .count();
    ...
    println!(
        "   Average Data Integrity: {:.1}%",
        avg_data_integrity * 100.0
    );
