...
impl BenchmarkResult {
    pub fn from_measurements(
        ...
        let min_time = *sorted_measurements
            .first()
            .unwrap_or(&Duration::from_nanos(0));
        let max_time = *sorted_measurements
            .last()
            .unwrap_or(&Duration::from_nanos(0));
...
mod loading_benchmarks {
    ...
    async fn benchmark_configuration_loading() {
        ...
        assert!(
            benchmark_result.meets_requirements,
            "Configuration loading benchmark failed to meet requirements"
        );
        assert!(
            benchmark_result.success_rate > 0.99,
            "Configuration loading success rate too low: {:.1}%",
    ...
    async fn benchmark_cached_configuration_loading() {
        ...
        assert!(
            benchmark_result.avg_time < Duration::from_millis(1),
            "Cached configuration loading too slow: {:?}",
            benchmark_result.avg_time
        );
        assert!(
            benchmark_result.success_rate == 1.0,
    ...
    async fn benchmark_large_configuration_loading() {
        ...
        assert!(
            benchmark_result.avg_time < Duration::from_millis(100),
            "Large configuration loading too slow: {:?}",
            benchmark_result.avg_time
        );
        assert!(
...
mod saving_benchmarks {
    ...
    async fn benchmark_configuration_saving() {
        ...
        for i in 0..100 {
            // Fewer iterations due to I/O cost
        ...
        assert!(
            benchmark_result.avg_time < Duration::from_millis(50),
            "Configuration saving too slow: {:?}",
            benchmark_result.avg_time
        );
        assert!(
            benchmark_result.success_rate > 0.99,
    ...
    async fn benchmark_concurrent_configuration_operations() {
        ...
        assert!(
            benchmark_result.avg_time < Duration::from_millis(10),
...
mod memory_benchmarks {
    ...
    async fn benchmark_memory_usage_scaling() {
        ...
        for (size, memory_mb) in &results {
            if *size <= 100 {
                assert!(
                    *memory_mb < MAX_MEMORY_USAGE_MB as f64,
                    "Memory usage for {} sections ({:.2} MB) exceeds limit",
                    size,
                    memory_mb
                );
            }
    ...
    async fn benchmark_memory_leak_detection() {
        ...
        if memory_measurements.len() >= 3 {
            ...
            assert!(
                stabilization_growth < 2.0,
                "Memory usage not stabilizing: {:.2} MB growth in steady state",
                stabilization_growth
            );
        }
    }
}
...
mod platform_benchmarks {
    ...
    async fn benchmark_platform_path_resolution() {
        ...
        assert!(
            benchmark_result.avg_time < Duration::from_micros(100),
            "Platform path resolution too slow: {:?}",
            benchmark_result.avg_time
        );
    }
    ...
    async fn benchmark_cross_platform_compatibility() {
        ...
        platform_section.insert(
            "supports_xdg".to_string(),
            ConfigValue::boolean(platform_info.supports_xdg),
        );
        config.set_section(
            "platform".to_string(),
            ConfigValue::object(platform_section),
        );
...
pub async fn run_benchmark_suite() -> Vec<BenchmarkResult> {
    ...
    println!(
        "Platform: {} ({}) - Arch: {}",
        platform_info.name, platform_info.version, platform_info.arch
    );
    println!(
        "Features: XDG={}, Keyring={}, FileWatch={}",
    ...
    println!(
        ...
        if total_benchmarks > 0 {
            requirements_met as f64 / total_benchmarks as f64 * 100.0
        } else {
            100.0
        }
    );
