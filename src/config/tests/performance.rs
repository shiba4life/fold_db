...
use super::{
    ...
    mocks::{create_large_test_config, MockPerformanceMonitor},
    utils::*,
};
...
mod loading_performance_tests {
    ...
    async fn test_configuration_load_performance() {
        ...
        assert!(
            avg_time < MAX_LOAD_TIME,
            "Average load time ({:?}) exceeds requirement ({:?})",
            avg_time,
            MAX_LOAD_TIME
        );
        assert!(
            results.memory_usage_mb < MAX_MEMORY_USAGE_MB as f64,
            "Memory usage ({:.2} MB) exceeds requirement ({} MB)",
    ...
    async fn test_cached_configuration_load_performance() {
        ...
        let avg_cached_time =
            cached_load_times.iter().sum::<Duration>() / cached_load_times.len() as u32;

        // Cached loads should be significantly faster than the 10ms requirement
        assert!(
            avg_cached_time < Duration::from_millis(1),
            "Cached load time ({:?}) should be much faster than requirement",
            avg_cached_time
        );

        println!(
            "📈 Cached load performance: avg {:?}, success rate: {:.1}%",
            avg_cached_time,
            success_count as f64 / PERF_TEST_ITERATIONS as f64 * 100.0
        );
    }
    ...
    async fn test_large_configuration_load_performance() {
        ...
        assert!(
            ...
            "Large configuration load time ({:?}) exceeds reasonable limit",
            load_time
        );
...
mod saving_performance_tests {
    ...
    async fn test_configuration_save_performance() {
        ...
        for i in 0..100 {
            // Fewer iterations for save tests (disk I/O intensive)
        ...
        assert!(
            ...
            "Average save time ({:?}) exceeds reasonable limit",
            avg_time
        );
        assert!(
            results.success_rate > 0.99,
            "Save success rate too low: {:.1}%",
            results.success_rate * 100.0
        );
    }
    ...
    async fn test_atomic_save_performance() {
        ...
        let avg_atomic_time =
            atomic_save_times.iter().sum::<Duration>() / atomic_save_times.len() as u32;

        println!(
            "🔒 Atomic save performance: avg {:?}, success rate: {:.1}%",
            avg_atomic_time,
...
mod hot_reload_performance_tests {
    ...
    async fn test_hot_reload_performance() {
        ...
        assert!(
            reload_time < MAX_HOT_RELOAD_TIME,
            "Hot reload time ({:?}) exceeds requirement ({:?})",
            reload_time,
            MAX_HOT_RELOAD_TIME
        );
    ...
    async fn test_cache_performance() {
        ...
        assert!(
            avg_hit_time < Duration::from_micros(100),
            "Cache hits too slow: {:?}",
            avg_hit_time
        );
    }
...
mod memory_performance_tests {
    ...
    async fn test_memory_usage_typical_config() {
        ...
        println!(
            "💾 Memory usage for typical config: {:.2} MB (delta: {:.2} MB)",
            final_memory, memory_delta
        );

        // Memory usage should be well under the 1MB requirement
        assert!(
            memory_delta < MAX_MEMORY_USAGE_MB as f64,
            "Memory usage ({:.2} MB) exceeds requirement ({} MB)",
            memory_delta,
            MAX_MEMORY_USAGE_MB
        );
    ...
    async fn test_memory_usage_large_config() {
        ...
        println!(
            "💾 Memory usage for large config: {:.2} MB (delta: {:.2} MB)",
            final_memory, memory_delta
        );

        // Large configurations may use more memory but should still be reasonable
        assert!(
            memory_delta < 5.0, // Allow up to 5MB for very large configs
            "Large config memory usage ({:.2} MB) exceeds reasonable limit",
            memory_delta
        );
    }
    ...
    async fn test_memory_leak_detection() {
        ...
        println!(
            "🔍 Memory leak test: baseline {:.2} MB, final {:.2} MB, growth {:.2} MB",
        ...
        assert!(
            memory_growth < 2.0,
            "Possible memory leak detected: {:.2} MB growth",
            memory_growth
        );
    }
}
...
mod concurrency_performance_tests {
    ...
    async fn test_concurrent_access_performance() {
        ...
        assert!(
            avg_time_per_op < Duration::from_millis(1),
    ...
    async fn test_mixed_read_write_performance() {
        ...
        assert!(
            ...
            "Mixed read/write test took too long: {:?}",
            total_time
        );
    }
}
...
fn create_test_configuration() -> Config {
    ...
    app_section.insert(
        "name".to_string(),
        ConfigValue::string("performance_test_app"),
    );
