...
use crate::config::{
    ...
    platform::{
        create_platform_resolver, get_platform_info, linux::LinuxConfigPaths,
        macos::MacOSConfigPaths, windows::WindowsConfigPaths, PlatformConfigPaths,
    },
    value::ConfigValue,
};
...
use super::{
    ...
    mocks::{MockAtomicOps, MockFileWatcher, MockKeystore, MockPlatformPaths},
    utils::*,
};
...
mod platform_compatibility_tests {
    ...
    async fn test_platform_specific_optimizations() {
        ...
        let test_configs = vec![
            (
                "linux",
                MockPlatformPaths::new(temp_dir.path().join("linux"), "linux"),
            ),
            (
                "macos",
                MockPlatformPaths::new(temp_dir.path().join("macos"), "macos"),
            ),
            (
                "windows",
                MockPlatformPaths::new(temp_dir.path().join("windows"), "windows"),
            ),
        ];
...
mod format_compatibility_tests {
    ...
    async fn test_toml_json_round_trip() {
        ...
        nested_obj.insert(
            ...
            ConfigValue::string("nested_value"),
        );
        ...
        let loaded_array = toml_loaded
            .get_value("complex.array_val")
            .unwrap()
            .as_array()
            .unwrap();
        ...
        assert_eq!(
            toml_loaded
                .get_value("complex.nested_obj.nested_key")
                .unwrap()
                .as_string()
                .unwrap(),
            "nested_value"
