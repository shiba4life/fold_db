...
mod tests {
    ...
    fn test_key_export_import_json_format() {
        ...
        let output = run_cli_command(&[
            ...
            "hex",
            "--private-key-file",
            &temp_dir.path().join("test_private.key").to_string_lossy(),
            "--public-key-file",
            &temp_dir.path().join("test_public.key").to_string_lossy(),
        ]);
        ...
        assert!(
            storage_dir.exists() || temp_dir.path().exists(),
    ...
    fn test_import_corrupted_file_detection() {
        ...
        let output = run_cli_command(&[
            "import-key",
            "--export-file",
            &corrupted_file.to_string_lossy(),
            "--key-id",
            "corrupted_key",
            "--storage-dir",
            &storage_dir.to_string_lossy(),
        ...
        assert!(
            ...
            "Import should fail for corrupted file"
        );
    }
    ...
    fn test_import_wrong_passphrase_detection() {
        ...
        fs::write(&export_file, valid_export).expect("Failed to create test export file");
        ...
        assert!(
            export_file.exists(),
            "Export file should exist for manual testing"
        );
    ...
    fn test_export_file_permissions() {
        ...
        if export_file.exists() {
            let metadata = fs::metadata(&export_file).expect("Failed to get export file metadata");
            ...
            #[cfg(unix)]
            {
                ...
                assert_eq!(
                    mode & 0o777,
                    0o600,
                    "Export file should have 600 permissions"
                );
            }
        }
    }
    ...
    fn test_metadata_preservation() {
        ...
        if export_file.exists() {
            ...
            if let Some(metadata) = parsed.get("metadata") {
                assert!(
                    metadata["key_id"].is_string(),
                    "Metadata should include original key_id"
                );
                assert!(
                    metadata["original_created"].is_string(),
                    "Metadata should include creation time"
                );
                assert!(
                    metadata["export_source"].is_string(),
                    "Metadata should include export source"
                );
            }
        }
    }
