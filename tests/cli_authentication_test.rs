...
fn test_cli_signing_config() {
    ...
    assert!(config.required_components.contains(&SignatureComponent::Method));
    assert!(config.required_components.contains(&SignatureComponent::TargetUri));
...
fn test_cli_settings() {
    ...
    assert!(sig_settings.default_components.contains(&"@method".to_string()));
    assert!(sig_settings.default_components.contains(&"@target-uri".to_string()));
    assert!(sig_settings.default_components.contains(&"content-type".to_string()));
}
