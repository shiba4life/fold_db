...
use datafold::datafold_node::signature_auth::{
    SignatureAuthConfig, SignatureVerificationState, SecurityProfile, NonceStoreStats
};
...
async fn test_nonce_format_validation_disabled() {
    ...
    assert!(state.check_and_store_nonce("another_nonce_456", now).is_ok());
    ...
    assert!(state.check_and_store_nonce("nonce@with#special$chars", now).is_err());
...
async fn test_rfc3339_timestamp_format_validation() {
    ...
    let strict_state = SignatureVerificationState::new(strict_config).expect("Config should be valid");
    ...
    assert!(strict_state.validate_rfc3339_timestamp("invalid-format").is_err());
}
