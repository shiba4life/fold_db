...
impl E2ETestFixture {
    ...
    fn get_unique_db_path(&self, suffix: &str) -> PathBuf {
        ...
        self.temp_dir
            .path()
            .join(format!("db_{}_{}", counter, suffix))
    }
    ...
    fn create_encryption_wrapper_with_path(
        &self,
        db_path: &PathBuf,
    ) -> Result<EncryptionWrapper, Box<dyn std::error::Error>> {
    ...
    fn create_backup_manager_with_path(
        &self,
        db_path: &PathBuf,
    ) -> Result<EncryptedBackupManager, Box<dyn std::error::Error>> {
        let db = sled::open(db_path)?;
        let db_ops = DbOperations::new(db)?;
        Ok(EncryptedBackupManager::new(db_ops, &self.master_keypair)?)
    }
...
fn test_e2e_complete_system_initialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ DataFoldNode initialized with crypto in {:?}",
        node_init_time
    );
    ...
    assert_eq!(
        derived_keys.len(),
        contexts_list.len(),
        "Should derive keys for all contexts"
    );
    ...
    println!(
        "✅ Key derivation system working for {} contexts",
        contexts_list.len()
    );
    ...
    assert!(
        encryption_wrapper.is_encryption_enabled(),
...
fn test_e2e_multi_context_encryption_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_data = vec![
        (
            contexts::ATOM_DATA,
            TestAtomData::new("atom1", "test_atom", vec![1, 2, 3, 4]),
        ),
        (
            contexts::SCHEMA_DATA,
            TestAtomData::new("schema1", "test_schema", vec![5, 6, 7, 8]),
        ),
        (
            contexts::METADATA,
            TestAtomData::new("metadata1", "test_metadata", vec![9, 10, 11, 12]),
        ),
        (
            contexts::TRANSFORM_DATA,
            TestAtomData::new("transform1", "test_transform", vec![100; 1024]),
        ),
    ];
    ...
    for (context, data) in &test_data {
        ...
        encryption_wrapper
            .store_encrypted_item(&key, data, context)
    ...
    for (context, expected_data) in &test_data {
        ...
        let retrieved: Option<TestAtomData> = encryption_wrapper
            .get_encrypted_item(&key, context)
            .map_err(|e| format!("Failed to retrieve from context {}: {}", context, e))?;

        assert!(
            retrieved.is_some(),
            "Should retrieve data from context {}",
            context
        );
        assert_eq!(
            &retrieved.unwrap(),
            expected_data,
            "Retrieved data should match in context {}",
            context
        );
...
fn test_e2e_backward_compatibility_and_migration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let unencrypted_data = vec![
        (
            "unencrypted_1",
            TestAtomData::new("u1", "unencrypted_atom_1", vec![1, 2, 3]),
        ),
        (
            "unencrypted_2",
            TestAtomData::new("u2", "unencrypted_atom_2", vec![4, 5, 6]),
        ),
    ];
    ...
    encryption_wrapper
        .store_encrypted_item("encrypted_1", &new_encrypted_data, contexts::ATOM_DATA)
        .map_err(|e| format!("Failed to store encrypted data: {}", e))?;

    // Should be able to read both encrypted and unencrypted data
    let retrieved_unencrypted: Option<TestAtomData> = encryption_wrapper
        .get_encrypted_item("unencrypted_1", contexts::ATOM_DATA)
        .map_err(|e| format!("Failed to read unencrypted data in gradual mode: {}", e))?;
    let retrieved_encrypted: Option<TestAtomData> = encryption_wrapper
        .get_encrypted_item("encrypted_1", contexts::ATOM_DATA)
        .map_err(|e| format!("Failed to read encrypted data: {}", e))?;

    assert!(
        retrieved_unencrypted.is_some(),
        "Should read unencrypted data"
    );
    assert!(retrieved_encrypted.is_some(), "Should read encrypted data");
    assert_eq!(
        &retrieved_encrypted.unwrap(),
        &new_encrypted_data,
    ...
    let migration_status = encryption_wrapper
        .get_migration_status()
        .map_err(|e| format!("Failed to get migration status: {}", e))?;
    assert!(
        migration_status.is_mixed_environment(),
        "Should be mixed environment"
    );
    assert!(
        !migration_status.is_fully_encrypted(),
        "Should not be fully encrypted yet"
    );
    assert!(
        migration_status.encryption_percentage() > 0.0,
        "Should have some encrypted data"
    );
    assert!(
        migration_status.encryption_percentage() < 100.0,
        "Should not be 100% encrypted"
    );
...
fn test_e2e_performance_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in &test_sizes {
        ...
        for i in 0..iterations {
            ...
            baseline_db_ops
                .db()
                .insert(key.as_bytes(), serialized.clone())?;
        }
    ...
    for size in &test_sizes {
        ...
        for i in 0..iterations {
            let key = format!("encrypted_{}_{}", size, i);
            let _retrieved: Option<TestAtomData> = encryption_wrapper
                .get_encrypted_item(&key, contexts::ATOM_DATA)
        ...
        println!(
            "📊 Encrypted {}-byte data: write={:?}, read={:?}",
            size, write_time, read_time
        );
    }
    ...
    for size in &test_sizes {
        ...
        let write_overhead = (encrypted_write.as_nanos() as f64 - baseline_write.as_nanos() as f64)
            / baseline_write.as_nanos() as f64
            * 100.0;
        let read_overhead = (encrypted_read.as_nanos() as f64 - baseline_read.as_nanos() as f64)
            / baseline_read.as_nanos() as f64
            * 100.0;

        println!(
            "📊 {}-byte data overhead: write={:.2}%, read={:.2}%",
            size, write_overhead, read_overhead
        );
        ...
        assert!(
            write_overhead < 1000000.0,
            "Write overhead should complete (got {:.2}%)",
            write_overhead
        );
        assert!(
            read_overhead < 1000000.0,
            "Read overhead should complete (got {:.2}%)",
            read_overhead
        );
    }
...
fn test_e2e_backup_and_restore_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, data) in test_data.iter().enumerate() {
        ...
        source_encryption_wrapper
            .store_encrypted_item(&key, data, contexts::ATOM_DATA)
            .map_err(|e| format!("Failed to store test data: {}", e))?;
    }

    println!(
        "✅ Stored {} test items in source database",
        test_data.len()
    );

    // Test 5.2: Create backup manager with same database (testing database isolation)
    let backup_manager = EncryptedBackupManager::new(
        source_encryption_wrapper.db_ops().clone(),
        &fixture.master_keypair,
    )?;
    ...
    assert_eq!(
        verified_metadata.backup_id,
        backup_result.metadata.backup_id
    );
    assert!(
        verified_metadata.encryption_params.encrypted,
    ...
    assert_eq!(
        restore_stats.error_count, 0,
        "Restore should have no errors"
    );

    // Test 5.6: Verify data accessibility in original database (testing isolation)
    for (i, expected_data) in test_data.iter().enumerate() {
        let key = format!("backup_test_{}", i);
        let retrieved: Option<TestAtomData> = source_encryption_wrapper
            .get_encrypted_item(&key, contexts::ATOM_DATA)
            .map_err(|e| format!("Failed to retrieve original data: {}", e))?;
        assert!(
            retrieved.is_some(),
            "Should retrieve original data for key {}",
            key
        );
        assert_eq!(
            &retrieved.unwrap(),
            expected_data,
            "Original data should remain intact for key {}",
            key
        );
    }
...
fn test_e2e_error_handling_and_recovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    encryption_wrapper
        .store_encrypted_item("valid_key", &test_data, contexts::ATOM_DATA)
    ...
    encryption_wrapper
        .db_ops()
        .db()
        .insert("corrupted_key".as_bytes(), corrupted_data)?;

    // Try to read corrupted data - should handle gracefully
    let result: Result<Option<TestAtomData>, _> =
        encryption_wrapper.get_encrypted_item("corrupted_key", contexts::ATOM_DATA);
    assert!(result.is_err(), "Should fail to read corrupted data");

    // Verify valid data is still readable
    let valid_result: Option<TestAtomData> = encryption_wrapper
        .get_encrypted_item("valid_key", contexts::ATOM_DATA)
    ...
    assert!(
        result.is_ok(),
        "KeyDerivationManager should handle different configurations"
    );
    ...
    encryption_wrapper
        .store_encrypted_item("recovery_key", &recovery_data, contexts::ATOM_DATA)
        .map_err(|e| format!("Failed to store recovery data: {}", e))?;

    let retrieved: Option<TestAtomData> = encryption_wrapper
        .get_encrypted_item("recovery_key", contexts::ATOM_DATA)
        .map_err(|e| format!("Failed to retrieve recovery data: {}", e))?;
    assert!(
        retrieved.is_some(),
        "Should store and retrieve data after error scenarios"
    );
    assert_eq!(
        retrieved.unwrap(),
        recovery_data,
...
fn test_e2e_configuration_and_flexibility() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mode in &migration_modes {
        ...
        let db_ops = DbOperations::new(db)?;
        let wrapper =
            EncryptionWrapper::with_migration_mode(db_ops, &fixture.master_keypair, *mode)?;
...
fn test_e2e_final_comprehensive_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut stored_test_data: std::collections::HashMap<String, TestAtomData> =
    ...
    for (dataset_name, item_size, count) in &test_datasets {
        ...
        for (i, item) in data.iter().enumerate() {
            for context in contexts::all_contexts() {
                ...
                encryption_wrapper
                    .store_encrypted_item(&key, item, context)
                    .map_err(|e| format!("Failed to store in context {}: {}", context, e))?;
    ...
    println!(
        "📊 Performance test: stored {} items in {:?}",
        perf_iterations, encrypted_time
    );
    ...
    println!(
        "✅ Backup system operates without database conflicts (file created: {})",
        backup_path.exists()
    );
    ...
    let migration_status = encryption_wrapper
        .get_migration_status()
        .map_err(|e| format!("Failed to get migration status: {}", e))?;
    let encryption_stats = encryption_wrapper
        .get_encryption_stats()
        .map_err(|e| format!("Failed to get encryption stats: {}", e))?;
    ...
    println!(
        "✅ Final comprehensive integration test passed in {:?}",
        total_time
    );
