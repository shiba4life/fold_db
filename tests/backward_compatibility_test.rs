...
use datafold::db_operations::{DbOperations, EncryptionWrapper, MigrationMode, MigrationConfig};
...
fn create_legacy_database() -> (sled::Db, DbOperations) {
    ...
    db_ops.store_item("legacy_user_1", &json!({
        "name": "Alice",
        "email": "alice@example.com",
        "age": 30
    })).unwrap();
    
    db_ops.store_item("legacy_user_2", &json!({
        "name": "Bob", 
        "email": "bob@example.com",
        "age": 25
    })).unwrap();
    
    db_ops.store_item("legacy_config", &json!({
        "version": "1.0.0",
...
fn create_encryption_wrapper_with_mode(db_ops: DbOperations, mode: MigrationMode) -> EncryptionWrapper {
...
fn test_read_only_compatibility_mode() {
    ...
    let user1: serde_json::Value = wrapper.get_encrypted_item("legacy_user_1", contexts::ATOM_DATA)
        .unwrap().unwrap();
    ...
    wrapper.store_encrypted_item("new_user", &json!({"name": "Charlie"}), contexts::ATOM_DATA).unwrap();
...
fn test_gradual_migration_mode() {
    ...
    let new_user: serde_json::Value = wrapper.get_encrypted_item("new_user", contexts::ATOM_DATA)
        .unwrap().unwrap();
...
fn test_full_migration_mode() {
    ...
    let user1_after: serde_json::Value = wrapper.get_encrypted_item("legacy_user_1", contexts::ATOM_DATA)
        .unwrap().unwrap();
...
fn test_mixed_environment_detection() {
    ...
    wrapper.store_encrypted_item("encrypted_user", &json!({"name": "Dave"}), contexts::ATOM_DATA).unwrap();
...
fn test_unencrypted_data_detection() {
    ...
    // Add encrypted data
    wrapper.store_encrypted_item("encrypted_item", &json!({"test": "data"}), contexts::ATOM_DATA).unwrap();
...
fn test_data_format_validation() {
    ...
    // Add some encrypted data
    wrapper.store_encrypted_item("encrypted_user", &json!({"name": "Eve"}), contexts::ATOM_DATA).unwrap();
    
    // Add some invalid data directly to the database
    wrapper.db_ops().db().insert("invalid_data", b"not json at all").unwrap();
    ...
    assert_eq!(validation_stats.get(&format!("context_{}", contexts::ATOM_DATA)), Some(&1));
}
...
fn test_enhanced_error_handling_read_only_mode() {
    ...
    wrapper.db_ops().db().insert("corrupted_data", b"invalid json").unwrap();
    
    // In read-only mode, should return None for corrupted data instead of error
    let result: Option<serde_json::Value> = wrapper.get_encrypted_item("corrupted_data", contexts::ATOM_DATA).unwrap();
...
fn test_enhanced_error_handling_strict_mode() {
    ...
    let result: Result<Option<serde_json::Value>, SchemaError> = wrapper.get_encrypted_item("corrupted_data", contexts::ATOM_DATA);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Failed to deserialize data"));
}

#[test]
fn test_migration_mode_switching() {
    let (_, db_ops) = create_legacy_database();
    let mut wrapper = create_encryption_wrapper_with_mode(db_ops, MigrationMode::ReadOnlyCompatibility);
...
fn test_batch_migration_with_verification() {
    ...
    for i in 0..10 {
        wrapper.db_ops().store_item(&format!("batch_item_{}", i), &json!({"id": i})).unwrap();
    }
    ...
    let batch_item: serde_json::Value = wrapper.get_encrypted_item("batch_item_5", contexts::ATOM_DATA)
        .unwrap().unwrap();
...
fn test_migration_with_different_contexts() {
    ...
    let user1: serde_json::Value = wrapper.get_encrypted_item("legacy_user_1", contexts::SCHEMA_DATA)
        .unwrap().unwrap();
    assert_eq!(user1["name"], "Alice");
    
    // Should fail with wrong context
    let result: Result<Option<serde_json::Value>, SchemaError> = wrapper.get_encrypted_item("legacy_user_1", contexts::ATOM_DATA);
    assert!(result.is_err());
}
...
fn test_tree_operations_backward_compatibility() {
    ...
    db_ops.store_in_tree(&db_ops.metadata_tree(), "legacy_metadata", &json!({"version": "1.0"})).unwrap();
    
    let wrapper = create_encryption_wrapper_with_mode(db_ops, MigrationMode::Gradual);
    
    // Should be able to read unencrypted data from tree
    let metadata: serde_json::Value = wrapper.get_encrypted_from_tree(
    ...
    wrapper.store_encrypted_in_tree(
        wrapper.db_ops().metadata_tree(),
        "new_metadata",
        &json!({"version": "2.0"}),
        contexts::METADATA
    ...
    let new_metadata: serde_json::Value = wrapper.get_encrypted_from_tree(
        wrapper.db_ops().metadata_tree(),
        "new_metadata",
        contexts::METADATA
    ).unwrap().unwrap();
...
fn test_comprehensive_encryption_stats() {
    ...
    // Add some encrypted data
    wrapper.store_encrypted_item("encrypted_1", &json!({"test": 1}), contexts::ATOM_DATA).unwrap();
    wrapper.store_encrypted_item("encrypted_2", &json!({"test": 2}), contexts::SCHEMA_DATA).unwrap();
    ...
    assert_eq!(stats.get("migration_mode"), Some(&(MigrationMode::Gradual as u64)));
}
...
fn test_full_migration_mode_requires_encryption() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("read-only compatibility mode"));
}