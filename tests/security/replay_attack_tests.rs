...
use datafold::datafold_node::signature_auth::{
    ...
    AuthenticationError, SecurityEventSeverity, NonceStoreStats
};
...
impl ReplayAttackTestRunner {
    ...
    fn create_attack_scenarios() -> Vec<AttackScenario> {
        vec![
            ...
            AttackScenario {
                ...
                concurrent_attackers: 2,
                use_valid_signatures: true,
                timestamp_manipulation: TimestampManipulation::FutureTimestamp { seconds_ahead: 120 },
                nonce_strategy: NonceStrategy::ValidUUID4,
            },
    ...
    pub async fn run_all_scenarios(&mut self) -> Result<Vec<ReplayAttackResult>, Box<dyn std::error::Error>> {
        ...
        for scenario in &self.scenarios.clone() {
            ...
            println!("  - Avg response time: {}ms", result.average_response_time_ms);
    ...
    async fn run_attack_scenario(&self, scenario: &AttackScenario) -> Result<ReplayAttackResult, Box<dyn std::error::Error>> {
        let state = self.states.get(&scenario.security_profile)
            ...
            for _attacker in 0..scenario.concurrent_attackers {
                ...
                match result {
                    Ok(_) => {
                        ...
                        if total_attempts > 1 && scenario.nonce_strategy == NonceStrategy::ExactReplay {
        ...
        let memory_impact = (final_stats.total_nonces - initial_stats.total_nonces) * 
            std::mem::size_of::<String>(); // Rough estimate
    ...
    fn generate_nonce(&self, strategy: &NonceStrategy) -> String {
        match strategy {
            ...
            NonceStrategy::PredictableSequence => {
                format!("predictable-{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs())
            }
...
impl ValidationReport {
    ...
    fn generate_recommendations(&mut self) {
        ...
        if self.scenario_results.iter().any(|r| r.average_response_time_ms > 500) {
            self.recommendations.push(
                "Performance degradation detected under attack - consider optimization".to_string()
            );
        }
        
        if self.scenario_results.iter().any(|r| r.memory_impact_bytes > 1024 * 1024) {
            self.recommendations.push(
                "High memory usage detected - consider nonce store cleanup optimization".to_string()
            );
        }
    }
}
...
mod tests {
    ...
    async fn test_timestamp_manipulation_detection() {
        ...
        let nonce = Uuid::new_v4().to_string();
        assert!(state.check_and_store_nonce(&nonce, future_timestamp).is_err());
        
        // Past timestamp beyond window should fail
        let past_timestamp = now - 200; // Beyond strict window
        let nonce2 = Uuid::new_v4().to_string();
        assert!(state.check_and_store_nonce(&nonce2, past_timestamp).is_err());
    }
    ...
    async fn test_security_profile_differences() {
        ...
        assert!(strict_state.check_and_store_nonce(&nonce1, old_timestamp).is_err());
        
        // Should succeed in lenient mode (600s + 120s = 720s window)  
        assert!(lenient_state.check_and_store_nonce(&nonce2, old_timestamp).is_ok());
    }
