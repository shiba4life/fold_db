...
use datafold::db_operations::{
    BackupError, BackupMode, BackupOptions, DbOperations, EncryptedBackupManager, RestoreOptions,
};
...
fn create_test_database() -> (DbOperations, tempfile::TempDir) {
    ...
    metadata_tree
        .insert(
            "test_atom_1",
            serde_json::to_vec(&json!({"id": 1, "data": "test1"})).unwrap(),
        )
        .expect("Failed to store metadata");
    metadata_tree
        .insert(
            "test_atom_2",
            serde_json::to_vec(&json!({"id": 2, "data": "test2"})).unwrap(),
        )
        .expect("Failed to store metadata");

    // Store in schemas tree
    let schemas_tree = db_ops
        .db()
        .open_tree("schemas")
        .expect("Failed to open schemas tree");
    schemas_tree
        .insert(
            "test_schema_1",
            serde_json::to_vec(&json!({"name": "TestSchema", "version": 1})).unwrap(),
        )
...
fn verify_restored_data(db_ops: &DbOperations) {
    ...
    let atom1_data = metadata_tree
        .get("test_atom_1")
        .expect("Failed to get test_atom_1");
    assert!(
        atom1_data.is_some(),
        "test_atom_1 should exist in restored database"
    );
    let atom1: serde_json::Value =
        serde_json::from_slice(&atom1_data.unwrap()).expect("Failed to deserialize atom1");
    assert_eq!(atom1["id"], 1);

    let atom2_data = metadata_tree
        .get("test_atom_2")
        .expect("Failed to get test_atom_2");
    assert!(
        atom2_data.is_some(),
        "test_atom_2 should exist in restored database"
    );
    let atom2: serde_json::Value =
        serde_json::from_slice(&atom2_data.unwrap()).expect("Failed to deserialize atom2");
    assert_eq!(atom2["id"], 2);

    let schema1_data = schemas_tree
        .get("test_schema_1")
        .expect("Failed to get test_schema_1");
    assert!(
        schema1_data.is_some(),
        "test_schema_1 should exist in restored database"
    );
    let schema1: serde_json::Value =
        serde_json::from_slice(&schema1_data.unwrap()).expect("Failed to deserialize schema1");
...
fn test_full_backup_creation() {
    ...
    assert_eq!(
        backup_result.metadata.encryption_params.algorithm,
        "AES-256-GCM"
    );
...
fn test_backup_restore_roundtrip() {
    ...
    let metadata_tree = original_db_ops
        .db()
        .open_tree("metadata")
        .expect("Failed to open metadata tree");
    let atom1_check = metadata_tree
        .get("test_atom_1")
        .expect("Failed to check test data");
    ...
    let backup_result = backup_manager
        .create_backup(backup_path, &backup_options)
        .expect("Failed to create backup");

    // Validate backup was created successfully
    assert!(
        backup_result.stats.items_backed_up > 0,
        "No items were backed up"
    );
    println!(
        "Backup created with {} items",
        backup_result.stats.items_backed_up
    );
    ...
    let restore_db_ops =
        DbOperations::new(restore_db).expect("Failed to create restore DbOperations");
    ...
    println!(
        "Restore completed with {} items",
        restore_stats.items_restored
    );
...
fn test_backup_with_tree_filter() {
    ...
    assert!(
        result.is_ok(),
        "Failed to create filtered backup: {:?}",
        result.err()
    );
