...
async fn demo_basic_replay_prevention() {
    ...
    for (profile_name, config) in profiles {
        ...
        println!(
            "  📊 {} profile: Replay protection working correctly",
            profile_name
        );
    }
}
    ...
    for (profile_name, config, scenarios) in test_cases {
        ...
        for (scenario_name, timestamp, should_succeed) in scenarios {
            ...
            if should_succeed {
                assert!(
                    result.is_ok(),
                    "Scenario '{}' should succeed in {} profile",
                    scenario_name,
                    profile_name
                );
                ...
                assert!(
                    result.is_err(),
                    "Scenario '{}' should fail in {} profile",
                    scenario_name,
                    profile_name
                );
...
async fn demo_nonce_format_validation() {
    ...
    for (i, nonce) in acceptable_nonces.iter().enumerate() {
        ...
        assert!(
            result.is_ok(),
            "Nonce '{}' should be accepted in lenient mode",
            nonce
        );
...
async fn demo_attack_scenarios() {
    ...
    state
        .check_and_store_nonce(&attack_nonce, timestamp)
        .unwrap();
    ...
    println!(
        "  ✅ All {} replay attempts successfully blocked",
        blocked_count
    );
    ...
    for (description, bad_timestamp) in manipulated_timestamps {
        let result = state.check_and_store_nonce(
            &format!("{}-{}", manipulation_nonce, description),
            bad_timestamp,
        );
        if result.is_err() {
    ...
    println!(
        "  ✅ Blocked {}/4 timestamp manipulation attempts",
        blocked_manipulation_count
    );
}
...
async fn demo_performance_under_attack() {
    ...
    println!(
        "  ⏱️ Baseline: 100 requests in {:?} (avg: {:.2}μs)",
    ...
    println!(
        "  ⏱️ Under attack: 100 replay attempts in {:?} (avg: {:.2}μs)",
        attack_duration, attack_avg
    );
    ...
    assert!(
        performance_impact < 500.0,
        "Performance impact should be reasonable (got {:.1}%)",
        performance_impact
    );
...
async fn demo_nonce_store_efficiency() {
    ...
    println!(
        "  📈 Final stats: {} nonces stored (max capacity: {})",
...
async fn demo_comprehensive_security_assessment() {
    ...
    for (profile_name, config) in profiles {
        ...
        println!(
            "  📊 Timestamp Validation Rate: {:.1}%",
            timestamp_validation_rate
        );
        ...
        assert!(
            overall_score >= 80.0,
            "{} profile should score at least 80%",
            profile_name
        );
    }
}
...
async fn demo_production_readiness_validation() {
    ...
    let nonce = Uuid::new_v4().to_string();
    strict_state
        .check_and_store_nonce(&nonce, timestamp)
        .unwrap();
    ...
    let baseline_timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs();
    ...
    let perf_timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs();
    let attack_nonce = Uuid::new_v4().to_string(); // Use proper UUID4 for strict profile
    strict_state
        .check_and_store_nonce(&attack_nonce, perf_timestamp)
        .unwrap();
    ...
    let profiles_timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs();
    ...
    let nonce_timestamp = SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let invalid_nonce_result =
        strict_state.check_and_store_nonce("invalid-format", nonce_timestamp);
    let valid_nonce_result =
        strict_state.check_and_store_nonce(&Uuid::new_v4().to_string(), nonce_timestamp + 1);
    ...
    if stats.total_nonces <= stats.max_capacity {
        passed_checks += 1;
        println!(
            "  ✅ PASS: Nonce store within capacity limits ({}/{})",
    ...
    assert!(
        ...
        "System must be at least 80% ready for production"
    );
    assert!(
        passed_checks >= 5,
        "Must pass at least 5 out of 6 critical checks"
    );
