...
fn test_store_and_retrieve_encrypted_simple_data() {
    ...
    // Store encrypted
    wrapper.store_encrypted_item(test_key, &test_string, context).unwrap();
...
fn test_store_and_retrieve_encrypted_complex_data() {
    ...
    // Retrieve and verify
    let retrieved: TestData = wrapper.get_encrypted_item(test_key, context).unwrap().unwrap();
...
fn test_multiple_encryption_contexts() {
    ...
    let result: Result<Option<String>, SchemaError> = wrapper.get_encrypted_item(test_key, contexts::SCHEMA_DATA);
...
fn test_backward_compatibility_unencrypted_data() {
    ...
    let retrieved: TestData = wrapper.get_encrypted_item(test_key, contexts::ATOM_DATA).unwrap().unwrap();
    assert_eq!(retrieved, test_data);
}
...
fn test_migration_to_encrypted() {
    ...
    let retrieved1: TestData = wrapper.get_encrypted_item("item1", contexts::ATOM_DATA).unwrap().unwrap();
    assert_eq!(retrieved1, data1);
    
    let retrieved2: TestData = wrapper.get_encrypted_item("item2", contexts::ATOM_DATA).unwrap().unwrap();
    assert_eq!(retrieved2, data2);
    
    let retrieved3: String = wrapper.get_encrypted_item("item3", contexts::ATOM_DATA).unwrap().unwrap();
...
fn test_migration_with_encryption_disabled() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("encryption is disabled"));
}
...
fn test_error_handling_invalid_context() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Unknown encryption context"));
}
...
fn test_error_handling_corrupted_data() {
    ...
    wrapper.db_ops().db().insert(test_key.as_bytes(), corrupted_data).unwrap();
    
    // Trying to retrieve should fail
    let result: Result<Option<String>, SchemaError> = wrapper.get_encrypted_item(test_key, contexts::ATOM_DATA);
    assert!(result.is_err());
}
...
async fn test_large_data_encryption() {
    ...
    let result = tokio::time::timeout(encryption_timeout, async {
        ...
        wrapper.store_encrypted_item(test_key, &large_data, contexts::ATOM_DATA).unwrap();
        
        // Retrieve and verify data
        let retrieved: Vec<u8> = wrapper.get_encrypted_item(test_key, contexts::ATOM_DATA).unwrap().unwrap();
    ...
    match result {
        Ok((retrieved, duration)) => {
            ...
            assert!(duration < std::time::Duration::from_secs(15), "Test took too long: {:?}", duration);
            println!("Large data encryption test completed in {:?}", duration);
        }
        Err(_) => {
            panic!("Test timed out after {} seconds - encryption operations are hanging", encryption_timeout.as_secs());
        }
    }
}
...
fn test_empty_data_encryption() {
    ...
    wrapper.store_encrypted_item(test_key, &empty_data, contexts::ATOM_DATA).unwrap();
    let retrieved: Vec<u8> = wrapper.get_encrypted_item(test_key, contexts::ATOM_DATA).unwrap().unwrap();
...
fn test_encryption_format_detection() {
    ...
    wrapper.store_encrypted_item(encrypted_key, &encrypted_data, contexts::ATOM_DATA).unwrap();
    
    // Store unencrypted data directly
    let unencrypted_data = "unencrypted test data";
    let unencrypted_key = "unencrypted_key";
    wrapper.db_ops().store_item(unencrypted_key, &unencrypted_data).unwrap();
    
    // Both should be readable through the wrapper
    let retrieved_encrypted: String = wrapper.get_encrypted_item(encrypted_key, contexts::ATOM_DATA).unwrap().unwrap();
    assert_eq!(retrieved_encrypted, encrypted_data);
    
    let retrieved_unencrypted: String = wrapper.get_encrypted_item(unencrypted_key, contexts::ATOM_DATA).unwrap().unwrap();
