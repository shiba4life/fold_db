...
fn test_query_layer_atomref_bug_reproduction() {
    ...
    println!(
        "✅ Schema created with static ref_atom_uuid: {}",
        initial_static_atom_uuid
    );
    ...
    let dynamic_aref_uuid = mutation_response
        .aref_uuid
    ...
    let dynamic_aref = db_ops
        .get_item::<datafold::atom::AtomRef>(&format!("ref:{}", dynamic_aref_uuid))
    ...
    println!(
        "   Bug: Query will try to find atom {}",
        initial_static_atom_uuid
    );
        ...
        Ok(value) => {
            ...
            if let Some(obj) = value.as_object() {
                if let Some(content) = obj.get("content") {
                    ...
                    println!(
                        "⚠️ Query returned object but missing 'content' field: {}",
                        value
                    );
                }
            } else {
    ...
    let dynamic_aref_uuid_2 = mutation_response_2
        .aref_uuid
        .expect("Should return same AtomRef UUID");

    // Should reuse the same AtomRef UUID
    assert_eq!(
        dynamic_aref_uuid, dynamic_aref_uuid_2,
        "Should reuse same AtomRef UUID"
    );
    ...
    println!(
        "✅ After second mutation, AtomRef points to atom: {}",
        updated_atom_uuid
    );
    assert_ne!(
    ...
    match TransformUtils::resolve_field_value(&Arc::new(db_ops), &test_schema, "test_field", None) {
        Ok(value) => {
            println!(
                "✅ Query layer returned value after second mutation: {}",
                value
            );
