...
use datafold::{
    CryptoConfig, MasterKeyConfig, KeyDerivationConfig, SecurityLevel,
...
fn test_node_config_serialization_without_crypto() {
    ...
    let deserialized: NodeConfig = serde_json::from_str(&json)
...
fn test_node_config_json_example_passphrase() {
    ...
    let config: NodeConfig = serde_json::from_str(json_config)
        .expect("Should parse JSON config");
    
    assert!(config.is_crypto_enabled());
    assert!(config.crypto_config().unwrap().requires_passphrase());
    assert_eq!(config.crypto_config().unwrap().key_derivation.preset, Some(SecurityLevel::Sensitive));
    assert!(config.validate().is_ok());
}
...
fn test_node_config_json_example_random_key() {
    ...
    assert!(config.is_crypto_enabled());
    assert!(config.crypto_config().unwrap().uses_random_key());
    assert_eq!(config.crypto_config().unwrap().key_derivation.preset, Some(SecurityLevel::Interactive));
    assert!(config.validate().is_ok());
}
...
fn test_node_config_backward_compatibility() {
    ...
    let config: NodeConfig = serde_json::from_str(json_config)
        .expect("Should parse old JSON config");
...
fn test_crypto_config_integration_with_argon2() {
    ...
    let crypto = config.crypto_config().unwrap();
    let argon2_params = crypto.key_derivation.to_argon2_params()
...
fn test_crypto_config_security_levels() {
    ...
    let argon2_params = config.crypto_config().unwrap()
        .key_derivation.to_argon2_params().unwrap();
...
fn test_external_key_source_config() {
    ...
    let config: NodeConfig = serde_json::from_str(json_config)
        .expect("Should parse external key config");
    
    assert!(config.is_crypto_enabled());
    let crypto = config.crypto_config().unwrap();
    assert!(matches!(crypto.master_key, MasterKeyConfig::External { .. }));
    assert!(config.validate().is_ok());
}
...
fn test_invalid_json_configs() {
    ...
    let config: NodeConfig = serde_json::from_str(json_config)
        .expect("Should parse JSON but fail validation");
