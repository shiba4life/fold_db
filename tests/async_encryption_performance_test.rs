...
async fn benchmark_sync_encryption(
    iterations: usize,
    data_size: usize,
...
async fn benchmark_async_encryption(
    iterations: usize,
    data_size: usize,
) -> Result<f64, Box<dyn std::error::Error>> {
    let key = [0x42u8; 32];
...
async fn benchmark_batch_encryption(
    batch_size: usize,
    data_size: usize,
) -> Result<f64, Box<dyn std::error::Error>> {
    let key = [0x42u8; 32];
...
async fn test_async_encryption_overhead() {
    let sync_ops_per_sec = benchmark_sync_encryption(BENCHMARK_ITERATIONS, SMALL_DATA_SIZE)
        .await
        .unwrap();
    let async_ops_per_sec = benchmark_async_encryption(BENCHMARK_ITERATIONS, SMALL_DATA_SIZE)
        .await
        .unwrap();
...
async fn test_large_data_encryption_performance() {
    let sync_ops_per_sec = benchmark_sync_encryption(50, LARGE_DATA_SIZE)
        .await
        .unwrap();
    let async_ops_per_sec = benchmark_async_encryption(50, LARGE_DATA_SIZE)
        .await
        .unwrap();
...
async fn test_batch_encryption_performance() {
    let single_ops_per_sec = benchmark_async_encryption(50, SMALL_DATA_SIZE)
        .await
        .unwrap();
    let batch_ops_per_sec = benchmark_batch_encryption(50, SMALL_DATA_SIZE)
        .await
        .unwrap();

    let improvement_percent =
        ((batch_ops_per_sec - single_ops_per_sec) / single_ops_per_sec) * 100.0;
...
async fn test_async_wrapper_performance() {
    ...
    let async_wrapper = AsyncEncryptionWrapper::new(db_ops, &master_keypair, async_config)
        .await
        .unwrap();
    ...
    for i in 0..iterations {
        ...
        sync_wrapper
            ...
            .unwrap();
        let _retrieved: Vec<u8> = sync_wrapper
    ...
    for i in 0..iterations {
        ...
        async_wrapper
            ...
            .await
            .unwrap();
        let _retrieved: Vec<u8> = async_wrapper
...
async fn test_async_atom_manager_performance() {
    ...
    let async_atom_manager =
        AsyncAtomManager::with_encryption(db_ops, &master_keypair, async_config)
            .await
            .unwrap();
    ...
    assert!(
        ops_per_sec >= 10.0,
        "Async atom manager performance ({:.2} ops/sec) is too low",
        ops_per_sec
    );
    ...
    let _batch_atoms = async_atom_manager
        .get_atoms_batch_async(batch_uuids)
        .await
        .unwrap();
    ...
    assert!(
        batch_ops_per_sec >= ops_per_sec / 2.0,
...
async fn test_concurrent_operations_performance() {
    ...
    assert!(
        ops_per_sec >= 50.0,
        "Concurrent operations performance ({:.2} ops/sec) is too low",
        ops_per_sec
    );
}
...
async fn test_performance_config_profiles() {
    ...
    for (name, config) in configs {
        ...
        assert!(
            ops_per_sec >= 10.0,
            "Config '{}' performance ({:.2} ops/sec) is too low",
            name,
            ops_per_sec
        );
    }
...
async fn test_comprehensive_performance_validation() {
    ...
    let failed_cases: Vec<_> = all_results
        .iter()
    ...
    if !failed_cases.is_empty() {
        ...
        for (size, results) in failed_cases {
            println!(
                "  {} bytes: {:.2}% overhead",
                size, results.overhead_percent
            );
        }
        panic!(
            "Some test cases exceeded the maximum allowed overhead of {}%",
            MAX_OVERHEAD_PERCENT
        );
    }

    // Calculate average overhead
    let avg_overhead = all_results
        .iter()
...
async fn test_pbi_9_performance_requirements() {
    ...
    let async_wrapper = AsyncEncryptionWrapper::new(db_ops, &master_keypair, config)
        .await
        .unwrap();
    ...
    let overhead_percent =
        ((baseline_ops_per_sec - encrypted_ops_per_sec) / baseline_ops_per_sec) * 100.0;

    println!("PBI 9 Performance Validation:");
    println!(
        "  Baseline (unencrypted) ops/sec: {:.2}",
