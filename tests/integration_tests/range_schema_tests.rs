...
fn test_range_schema_multiple_mutations_same_key() {
    ...
    mutation1_fields.insert("score".to_string(), json!({
    ...
    mutation3_fields.insert("score".to_string(), json!({
...
fn create_user_scores_schema() -> Schema {
    ...
    for field_name in &["user_id", "game_scores", "achievements", "player_statistics", "ranking_data"] {
        let range_field = RangeField::new(
            PermissionsPolicy::default(),
            FieldPaymentConfig::default(),
            HashMap::new(),
...
fn test_user_scores_schema_complex_mutations() {
    ...
    fields.insert("player_statistics".to_string(), json!({
        "gamer_alice": {
            "total_games": 1247,
    ...
    fields.insert("ranking_data".to_string(), json!({
        "gamer_alice": {
            "global_rank": 1542,
            "regional_rank": 89,
            "tier": "Diamond",
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_ok(), "Complex UserScores mutation should succeed: {:?}", result);
    ...
    let stats = user_data.get("player_statistics").unwrap().as_object().unwrap();
...
fn test_range_schema_query_performance_large_dataset() {
    ...
    for user_id in &test_cases {
        ...
        assert_eq!(grouped_obj.len(), 1, "Should only contain data for queried user");
    }
}
...
fn test_range_schema_different_data_types_comprehensive() {
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_ok(), "Mutation with comprehensive data types should succeed: {:?}", result);
    ...
    let result = results[0].as_ref().unwrap();
    let grouped_obj = result.as_object().unwrap();
    let user_data = grouped_obj.get("data_types_test").unwrap().as_object().unwrap();
    let game_scores = user_data.get("game_scores").unwrap().as_object().unwrap();
    
    // Verify all data types
    assert_eq!(game_scores.get("integer_score").unwrap().as_i64(), Some(12345));
    assert_eq!(game_scores.get("float_score").unwrap().as_f64(), Some(98.75));
    assert_eq!(game_scores.get("string_score").unwrap().as_str(), Some("SSS+"));
    assert_eq!(game_scores.get("boolean_completed").unwrap().as_bool(), Some(true));
    assert_eq!(game_scores.get("false_boolean").unwrap().as_bool(), Some(false));
    assert!(game_scores.get("null_value").unwrap().is_null());
    assert_eq!(game_scores.get("zero_value").unwrap().as_i64(), Some(0));
    assert_eq!(game_scores.get("negative_value").unwrap().as_i64(), Some(-500));
    assert_eq!(game_scores.get("unicode_string").unwrap().as_str(), Some("🎮 Gaming Score! 日本語"));
    ...
    let nested_obj = game_scores.get("object_nested").unwrap().as_object().unwrap();
    ...
    assert!(game_scores.get("object_empty").unwrap().as_object().unwrap().is_empty());
    assert!(game_scores.get("array_empty").unwrap().as_array().unwrap().is_empty());
}
...
fn test_range_schema_edge_cases_and_boundary_conditions() {
    ...
    for user_id in &edge_case_users {
        ...
        let result = fold_db.write_schema(mutation);
        assert!(result.is_ok(), "Edge case user_id '{}' should work: {:?}", user_id, result);
        ...
        let result = results[0].as_ref().unwrap();
        let grouped_obj = result.as_object().unwrap();
        assert!(grouped_obj.contains_key(*user_id), "Should contain data for edge case user '{}'", user_id);
    }
}
...
fn test_range_schema_concurrent_operations_stress_test() {
    ...
    for i in 0..50 {
        ...
        let mutation = Mutation::new(
            ...
            "test_pubkey".to_string(),
            0,
            if i == 0 { MutationType::Create } else { MutationType::Update },
        );
...
fn test_range_schema_query_isolation_between_users() {
    ...
    for (target_user, expected_secret, expected_score) in &users_data {
        ...
        assert!(grouped_obj.contains_key(*target_user), "Should contain target user");
        
        // Verify no other users' data is present
        for (other_user, _, _) in &users_data {
            if other_user != target_user {
                assert!(!grouped_obj.contains_key(*other_user), 
                       "Should not contain other user '{}' when querying for '{}'", other_user, target_user);
            }
        }
        ...
        let game_scores = user_data.get("game_scores").unwrap().as_object().unwrap();
        assert_eq!(game_scores.get("secret_score").unwrap().as_i64(), Some(*expected_score as i64));
        assert_eq!(game_scores.get("secret_data").unwrap().as_str(), Some(*expected_secret));
    }
}