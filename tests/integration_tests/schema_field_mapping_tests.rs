...
fn test_field_mappers_share_aref_uuid() {
    ...
    assert!(
        source_ref_atom_uuid.is_some(),
        "Source field should have a ref_atom_uuid"
    );
    assert!(
        target_ref_atom_uuid.is_some(),
        "Target field should have a ref_atom_uuid"
    );

    // Verify that they are the same
    assert_eq!(
        source_ref_atom_uuid, target_ref_atom_uuid,
    ...
    node.execute_operation(serde_json::from_value(mutation).unwrap())
        .unwrap();
    ...
    let result1 = node
        .execute_operation(serde_json::from_value(query1).unwrap())
        .unwrap();
    let result2 = node
        .execute_operation(serde_json::from_value(query2).unwrap())
        .unwrap();
    ...
    assert_eq!(
        value1, "testuser",
        "Source field should contain the value we set"
    );
    assert_eq!(
        value2, "testuser",
        "Target field should contain the same value as the source field"
    );
}
