...
fn test_database_crypto_init_with_random_key() {
    ...
    let status = node.get_crypto_status().expect("Failed to get crypto status");
    ...
    let needs_init = node.is_crypto_init_needed(Some(&crypto_config))
...
fn test_database_crypto_init_with_passphrase() {
    ...
    assert!(status.derivation_method.as_ref().unwrap().starts_with("Argon2id-"));
}
...
fn test_crypto_already_initialized_error() {
    ...
    assert!(result.unwrap_err().to_string().contains("already has crypto metadata"));
}
...
fn test_different_security_levels() {
    ...
    for security_level in &security_levels {
        ...
        let node = DataFoldNode::new(config)
            .expect(&format!("Failed to create node with security level: {:?}", security_level));
        
        let status = node.get_crypto_status()
...
fn test_crypto_init_with_different_passphrase_lengths() {
    let test_cases = [
        ...
        ("very_long_passphrase_that_should_definitely_work_fine", false),
        ("", true), // Should fail
    ];
    ...
    for (passphrase, should_fail) in &test_cases {
        ...
        if *should_fail {
            assert!(result.is_err(), "Expected failure for passphrase: '{}'", passphrase);
        } else {
            assert!(result.is_ok(), "Expected success for passphrase: '{}'", passphrase);
...
fn test_crypto_init_performance_different_security_levels() {
    ...
    for security_level in &security_levels {
        ...
        println!("Crypto init with {:?} security level took: {:?}", security_level, duration);
        ...
        match security_level {
            SecurityLevel::Balanced => {
                ...
                assert!(duration.as_secs() < 5, "Balanced security level took too long: {:?}", duration);
            }
            SecurityLevel::Interactive => {
                // Should be reasonable for interactive use (under 10 seconds)
                assert!(duration.as_secs() < 10, "Interactive security level took too long: {:?}", duration);
            }
            SecurityLevel::Sensitive => {
                // Can be slow, but should complete within reasonable time
                assert!(duration.as_secs() < 30, "Sensitive security level took too long: {:?}", duration);
            }
        }
    }
}
