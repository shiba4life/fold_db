...
fn create_performance_test_config() -> SignatureAuthConfig {
    SignatureAuthConfig {
        ...
        required_signature_components: vec!["@method".to_string(), "@target-uri".to_string()],
...
async fn test_nonce_store_performance_under_load() {
    ...
    for i in 0..num_operations {
        ...
        assert!(
            result.is_ok(),
            "Nonce storage should succeed for unique nonce {}",
            i
        );
    }
    ...
    println!(
        "Average time per nonce operation: {} nanoseconds",
        avg_time_per_op
    );
    ...
    assert!(
        avg_time_per_op < 200_000,
        "Average nonce operation time {} ns exceeds 200μs target",
        avg_time_per_op
    );
}
...
async fn test_nonce_store_cleanup_efficiency() {
    ...
    let old_timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_secs()
        - 400; // Old enough to be cleaned up
...
async fn test_concurrent_nonce_operations() {
    ...
    assert!(
        ops_per_sec > 100.0,
        "Throughput {} ops/sec is below 100 ops/sec target",
        ops_per_sec
    );
}
...
async fn test_performance_monitoring_metrics() {
    ...
    let metrics = state
        ...
        .get_enhanced_security_metrics(1000);
...
async fn test_nonce_store_size_limit_enforcement() {
    ...
    let stats = state.get_nonce_store_stats().unwrap();
    println!(
        "After exceeding limit - Total nonces: {}, Max: {}",
        stats.total_nonces, stats.max_capacity
    );
...
async fn benchmark_signature_auth_performance() {
    ...
    assert!(
        ...
        "Average operation time {:?} exceeds 10ms target",
        avg_time
    );

    assert!(
        ops_per_sec > 100.0,
        "Throughput {:.2} ops/sec is below 100 ops/sec target",
        ops_per_sec
    );
...
async fn test_performance_improvement_validation() {
    ...
    println!(
        "   - Target <10ms: {}",
        if duration / 100 < Duration::from_millis(10) {
            "PASSED"
        } else {
            "FAILED"
        }
    ...
    println!(
        "   - Nonce store monitoring: {} nonces",
        metrics.nonce_store_size
    );
    ...
    assert!(
        duration / 100 < Duration::from_millis(10),
