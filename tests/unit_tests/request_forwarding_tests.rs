...
async fn test_request_forwarding() {
    ...
    node1.start_network_with_address("/ip4/127.0.0.1/tcp/9001").await.unwrap();
    node2.start_network_with_address("/ip4/127.0.0.1/tcp/9002").await.unwrap();
    ...
    let mut stream = tokio::net::TcpStream::connect("127.0.0.1:8001").await.unwrap();
