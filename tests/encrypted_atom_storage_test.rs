...
fn test_folddb_encryption_with_crypto_config() {
    ...
    folddb
        .enable_atom_encryption_with_config(&master_keypair, &crypto_config)
        .unwrap();
    assert!(folddb.is_atom_encryption_enabled());
}
...
fn test_encrypted_atom_creation_and_retrieval() {
    ...
    assert!(
        raw_data.starts_with(b"DF_ENC"),
        "Atom data should be encrypted"
    );
}
...
fn test_encrypted_atom_ref_operations() {
    ...
    let atom = folddb
        .atom_manager()
        .create_atom("test_schema", "test_pub_key".to_string(), test_content)
        .unwrap();
    ...
    let raw_ref_data = folddb
        .db_ops()
        .db()
        .get(ref_key.as_bytes())
        .unwrap()
        .unwrap();
    assert!(
        raw_ref_data.starts_with(b"DF_ENC"),
        "AtomRef data should be encrypted"
    );
}
...
fn test_encrypted_atom_ref_range_operations() {
    ...
    let raw_range_data = folddb
        .db_ops()
        .db()
        .get(range_key.as_bytes())
        .unwrap()
        .unwrap();
    assert!(
        raw_range_data.starts_with(b"DF_ENC"),
        "AtomRefRange data should be encrypted"
    );
}
...
fn test_backward_compatibility_with_unencrypted_atoms() {
    ...
    // Now open a database with encryption enabled
    {
        ...
        let _new_atom = folddb
            .atom_manager()
            .create_atom("test_schema", "test_pub_key".to_string(), new_content)
            .unwrap();
        ...
        let total_items =
            stats.get("encrypted_items").unwrap() + stats.get("unencrypted_items").unwrap();
...
fn test_migration_to_encrypted() {
    ...
    assert!(
        initial_unencrypted > 0,
        "Should have unencrypted items to migrate"
    );
    ...
    assert!(
        final_encrypted >= migrated_count,
        "Should have encrypted items"
    );
    assert!(
        final_unencrypted < initial_unencrypted,
        "Should have fewer unencrypted items"
    );
}
...
fn test_atom_history_with_encryption() {
    ...
    let atom1 = folddb
        .atom_manager()
        .create_atom("test_schema", "test_pub_key".to_string(), initial_content)
        .unwrap();
    ...
    let history = folddb
        .atom_manager()
        .get_atom_history("history_test_ref")
        .unwrap();
...
fn test_large_encrypted_atom_performance() {
    ...
    assert!(
        raw_data.starts_with(b"DF_ENC"),
        "Large atom should be encrypted"
    );
}
...
fn test_concurrent_encrypted_operations() {
    ...
    for i in 0..5 {
        ...
        let atom = folddb
            .atom_manager()
            .create_atom("concurrent_schema", format!("pub_key_{}", i), test_content)
            .unwrap();

        // Create corresponding atom reference
        let _atom_ref = folddb
            .atom_manager()
            .update_atom_ref(
    ...
    assert!(
        encrypted_items >= 10,
        "Should have created multiple encrypted items"
    ); // 5 atoms + 5 refs
}
