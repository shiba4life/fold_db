...
use datafold::datafold_node::crypto_init::{initialize_database_crypto, get_crypto_init_status};

/// Helper function to create a temporary node configuration with crypto settings
fn create_test_node_config_with_crypto(crypto_config: CryptoConfig) -> (NodeConfig, tempfile::TempDir) {
...
async fn test_crypto_init_with_different_security_levels() {
    for security_level in &[SecurityLevel::Interactive, SecurityLevel::Balanced, SecurityLevel::Sensitive] {
