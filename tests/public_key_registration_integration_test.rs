...
use datafold::datafold_node::{
    ...
    NodeConfig,
    crypto_routes::{
        PublicKeyRegistration, PUBLIC_KEY_REGISTRATIONS_TREE, CLIENT_KEY_INDEX_TREE,
    },
};
...
async fn test_store_and_retrieve_public_key_registration() {
    ...
    // Store registration
    fixture.store_test_registration(registration_id, client_id, public_key_bytes)
        .expect("Failed to store test registration");

    // Retrieve by registration ID
    let registration_key = format!("{}:{}", PUBLIC_KEY_REGISTRATIONS_TREE, registration_id);
    let retrieved_registration: PublicKeyRegistration = db_ops.get_item(&registration_key)
    ...
    let client_index_key = format!("{}:{}", CLIENT_KEY_INDEX_TREE, client_id);
    let retrieved_reg_id: String = db_ops.get_item(&client_index_key)
...
async fn test_public_key_validation() {
    ...
    assert!(public_key_result.is_ok(), "Valid public key should parse successfully");
    ...
    assert!(invalid_public_key_result.is_err(), "Invalid public key should fail to parse");
}
...
async fn test_duplicate_detection() {
    ...
    fixture.store_test_registration("reg-1", "client-1", public_key_bytes)
...
async fn test_registration_serialization() {
    ...
    let deserialized: PublicKeyRegistration = serde_json::from_slice(&serialized)
...
async fn test_database_operations() {
    ...
    db_ops.store_item(test_key, &test_value.to_string())
        .expect("Store operation should succeed");

    let retrieved_value: String = db_ops.get_item(test_key)
...
async fn test_multiple_registrations() {
    ...
    fixture.store_test_registration("reg-1", "client-1", key1)
        .expect("Registration 1 should succeed");
    fixture.store_test_registration("reg-2", "client-2", key2)
        .expect("Registration 2 should succeed");
    fixture.store_test_registration("reg-3", "client-3", key3)
        .expect("Registration 3 should succeed");

    // Verify all registrations exist
    let db_ops = fixture.db_ops();
    
    let reg1: PublicKeyRegistration = db_ops.get_item(&format!("{}:reg-1", PUBLIC_KEY_REGISTRATIONS_TREE))
        .expect("Get reg1 should succeed").expect("Reg1 should exist");
    let reg2: PublicKeyRegistration = db_ops.get_item(&format!("{}:reg-2", PUBLIC_KEY_REGISTRATIONS_TREE))
        .expect("Get reg2 should succeed").expect("Reg2 should exist");
    let reg3: PublicKeyRegistration = db_ops.get_item(&format!("{}:reg-3", PUBLIC_KEY_REGISTRATIONS_TREE))
        .expect("Get reg3 should succeed").expect("Reg3 should exist");
