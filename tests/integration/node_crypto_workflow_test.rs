...
impl NodeCryptoWorkflowFixture {
    ...
    fn config_with_passphrase_crypto(
        &self,
        passphrase: &str,
...
async fn test_complete_node_creation_workflow_with_random_crypto() {
    ...
    .enumerate()
    {
        ...
        println!(
            "✅ Node creation with {:?} security level successful",
            security_level
        );
    }
}
...
async fn test_complete_node_creation_workflow_with_passphrase_crypto() {
    ...
    for (i, (passphrase, security_level)) in test_cases.iter().enumerate() {
        ...
        // but successful creation implies the crypto initialization worked
        println!(
            "✅ Node creation with passphrase ({} chars) and {:?} security level successful",
            passphrase.len(),
            security_level
        );
    }
...
async fn test_crypto_initialization_error_handling() {
    ...
    assert!(
        result.is_err(),
        "Node creation should fail with empty passphrase"
    );

    let error_msg = result.err().unwrap().to_string();
    assert!(
        error_msg.contains("Crypto config validation failed"),
...
async fn test_crypto_workflow_performance_baseline() {
    ...
    let baseline_duration = {
        ...
        let _node_no_crypto =
            DataFoldNode::new(config).expect("Failed to create node without crypto");
        start.elapsed()
    };
    ...
    assert!(
        random_crypto_duration < max_acceptable_overhead,
        "Random crypto initialization should not add excessive overhead"
    );
    assert!(
        passphrase_crypto_duration < max_acceptable_overhead,
