    ...
    fn mutate_field_value(
        ...
        match self.db_ops.get_item::<datafold::atom::AtomRef>(&aref_key) {
            Ok(Some(aref)) => {
                ...
                println!(
                    "🔍 DIAGNOSTIC: AtomRef {} points to atom {}",
                    aref_uuid, atom_uuid
                );
...
fn test_single_field_mutation_to_query_flow() {
    ...
    let fixture = MutationQueryTestFixture::new().expect("Failed to create test fixture");
    ...
    let aref_uuid = fixture
        .mutate_field_value("TransformBase", "value1", test_value.clone(), "test_source")
        .expect("Failed to mutate value1");

    // Step 2: Query the same field and verify it returns the mutated value
    let result = fixture
        .query_field_value(&schema, "value1", &test_value)
    ...
    for (i, (value, source)) in mutation_cycles.iter().enumerate() {
        ...
        if i > 0 {
            ...
            assert_eq!(
                aref_uuids[i],
                aref_uuids[0],
                "Cycle {}: Should reuse same AtomRef UUID",
                i + 1
            );
        }
...
fn test_concurrent_mutations_and_queries() {
    ...
    for (i, success, aref_uuid, _value) in &results {
        ...
        assert!(
            aref_uuid.is_some(),
            "Concurrent operation {} should return AtomRef UUID",
            i
        );
        println!(
            "✅ Concurrent operation {} succeeded - AtomRef: {:?}",
            i, aref_uuid
        );
    }
    ...
    let final_result =
        TransformUtils::resolve_field_value(&fixture.db_ops, &schema, "value1", None)
...
fn test_diagnostic_atomref_bug_prevention() {
    ...
    let dynamic_aref = fixture
        .db_ops
        .get_item::<datafold::atom::AtomRef>(&aref_key)
    ...
    println!(
        "   Query correctly used dynamic AtomRef: {}",
        dynamic_aref_uuid
    );
...
fn test_complete_mutation_query_integration_workflow() {
    ...
    fixture
        .query_field_value(&schema, "value1", &complex_update)
    ...
    let final_value2 =
        TransformUtils::resolve_field_value(&fixture.db_ops, &schema, "value2", None)
