...
impl EndToEndWorkflowFixture {
    ...
    fn create_workflow_schemas(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut user_schema = Schema::new_range("UserProfile".to_string(), "user_id".to_string());
        ...
        self.db_ops
            .store_schema("UserAnalytics", &analytics_schema)?;

        // Schema 3: ActivityLog (range-based data)
        let mut activity_schema =
            Schema::new_range("ActivityLog".to_string(), "timestamp".to_string());
        ...
        self.db_ops
            .store_schema("DashboardSummary", &dashboard_schema)?;
    ...
    fn create_workflow_transforms(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.db_ops
            .store_transform("compute_full_name", &full_name_transform)?;
        ...
        age_category_transform.set_inputs(vec!["UserProfile.age".to_string()]);

        self.db_ops
            .store_transform("categorize_age", &age_category_transform)?;
        ...
        self.db_ops
            .store_transform("calculate_profile_score", &profile_score_transform)?;
...
fn test_complete_mutation_to_query_workflow() {
    ...
    let fixture = EndToEndWorkflowFixture::new().expect("Failed to create test fixture");
    ...
    for (field_name, value) in user_data {
        let aref_uuid = fixture
            .mutate_range_field_simple("UserProfile", field_name, value.clone())
    ...
    for (field_name, _aref_uuid, expected_value) in &mutation_results {
        let result = fixture
            .query_field_value("UserProfile", field_name)
        ...
        if let Some(range_map) = result.as_object() {
            ...
            let (key, stored_value) = range_map
                .iter()
                .next()
                .expect("Range should have at least one entry");
            assert_eq!(
                stored_value, expected_value,
                "Query result should match mutated value for {} (key: {})",
                field_name, key
            );
            println!(
                "✅ Query verified for {}: {} = {}",
                field_name, key, stored_value
            );
        } else {
            panic!(
                "Expected range field to return an object for {}",
                field_name
            );
        }
    }
    ...
    for (field_name, _aref_uuid, expected_value) in &mutation_results {
        ...
        if let Some(range_map) = result.as_object() {
            ...
            assert_eq!(
                stored_value, expected_value,
                "Persistent data should match for {} (key: {})",
                field_name, key
            );
        } else {
            panic!(
...
fn test_multi_schema_dependency_chain() {
    ...
    for (user_id, first_name, last_name, age, email) in &users {
        ...
        fixture
            .mutate_range_field("UserProfile", "first_name", user_id, json!(first_name))
            .expect("Failed to set first_name");

        fixture
            .mutate_range_field("UserProfile", "last_name", user_id, json!(last_name))
            .expect("Failed to set last_name");

        fixture
            .mutate_range_field("UserProfile", "age", user_id, json!(age))
            .expect("Failed to set age");

        fixture
            .mutate_range_field("UserProfile", "email", user_id, json!(email))
    ...
    let activities = vec![
        (
            ...
            "login",
            json!({"user": "user1", "ip": "192.168.1.1"}),
        ),
        (
            "2024-01-01T10:30:00Z",
            "view_profile",
            json!({"user": "user1", "page": "profile"}),
        ),
        (
            "2024-01-01T11:00:00Z",
            "login",
            json!({"user": "user2", "ip": "192.168.1.2"}),
        ),
        (
            "2024-01-01T11:15:00Z",
            "edit_profile",
            json!({"user": "user2", "changes": ["email"]}),
        ),
        (
            "2024-01-01T12:00:00Z",
            "login",
            json!({"user": "user3", "ip": "192.168.1.3"}),
        ),
    ];
    ...
    let user_first_names = fixture
        .query_field_value("UserProfile", "first_name")
    ...
    let activity_data = fixture
        .query_field_value("ActivityLog", "activity_type")
...
fn test_complex_data_transformation_scenarios() {
    ...
    fixture
        .db_ops
        .store_schema("OrderData", &order_schema)
    ...
    fixture
        .db_ops
        .store_schema("OrderSummary", &order_summary_schema)
    ...
    let retrieved_items = fixture
        .query_field_value("OrderData", "items")
        .expect("Failed to query items");

    assert_eq!(retrieved_items, order_data["items"]);

    let retrieved_address = fixture
        .query_field_value("OrderData", "shipping_address")
    ...
    let mut analytics_range_schema =
        Schema::new_range("RealtimeAnalytics".to_string(), "timestamp".to_string());
    ...
    fixture
        .db_ops
        .store_schema("RealtimeAnalytics", &analytics_range_schema)
    ...
    let analytics_points = vec![
        (
            ...
            json!({"page_views": 1523, "unique_visitors": 342, "bounce_rate": 0.45}),
        ),
        (
            "2024-01-01T01:00:00Z",
            json!({"page_views": 1876, "unique_visitors": 401, "bounce_rate": 0.42}),
        ),
        (
            "2024-01-01T02:00:00Z",
            json!({"page_views": 2134, "unique_visitors": 523, "bounce_rate": 0.38}),
        ),
        (
            "2024-01-01T03:00:00Z",
            json!({"page_views": 1987, "unique_visitors": 478, "bounce_rate": 0.41}),
        ),
    ];
    ...
    let metrics_result = fixture
        .query_field_value("RealtimeAnalytics", "metrics")
...
fn test_error_recovery_scenarios() {
    ...
    let invalid_field_result =
        fixture.mutate_range_field_simple("UserProfile", "non_existent_field", json!("test_value"));
    ...
    for (i, corrupt_data) in corruption_attempts.iter().enumerate() {
        ...
        match corruption_result {
            Ok(_) => {
                println!(
                    "⚠️  Corruption attempt {} succeeded, checking data integrity",
                    i + 1
                );
    ...
    println!(
        "✅ Concurrent operations handled: {} successful out of {}",
...
fn test_workflow_performance_characteristics() {
    ...
    println!(
        "✅ Bulk insert completed: {} items in {:?}",
        bulk_data_count, bulk_insert_duration
    );
    println!(
        "   Average per item: {:?}",
        bulk_insert_duration / bulk_data_count
    );
    ...
    println!(
        "✅ Query performance test completed: {} queries in {:?}",
        query_count, query_duration
    );
    ...
    for i in 0..mixed_operations {
        ...
        if i % 2 == 0 {
            ...
            // Query
            let _result = fixture
                .query_field_value("UserProfile", "last_name")
    ...
    println!(
        "✅ Mixed operations test completed: {} operations in {:?}",
        mixed_operations, mixed_duration
    );
    println!(
        "   Average per operation: {:?}",
        mixed_duration / mixed_operations
    );
    ...
    assert!(
        ...
        "Mutation performance should be > 1 ops/sec"
    );
    assert!(
        queries_per_second > 1.0,
        "Query performance should be > 1 ops/sec"
    );
