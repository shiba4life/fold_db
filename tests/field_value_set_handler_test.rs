...
fn test_field_value_set_request_handler() {
    ...
    assert!(
        response.aref_uuid.is_some(),
        "Should return an AtomRef UUID"
    );
...
fn test_field_value_set_statistics() {
    ...
    assert_eq!(
        final_stats.atom_refs_created,
        initial_refs + 1,
        "Should increment atom refs created"
    );
    ...
    println!(
        "   AtomRefs created: {} -> {}",
        initial_refs, final_stats.atom_refs_created
    );
}
