...
fn test_cli_generate_key_basic() {
    ...
    assert!(stdout.contains("# private key"), "Missing private key output");
    ...
    let private_key_line = lines.iter().find(|line| !line.starts_with('#') && line.len() == 64);
    let public_key_line = lines.iter().find(|line| !line.starts_with('#') && line.len() == 64 && *line != private_key_line.unwrap_or(&""));
    
    assert!(private_key_line.is_some(), "Private key not found in expected format");
    assert!(public_key_line.is_some(), "Public key not found in expected format");
    
    // Validate hex format
    let private_hex = private_key_line.unwrap();
    let public_hex = public_key_line.unwrap();
    
    assert!(private_hex.chars().all(|c| c.is_ascii_hexdigit()), "Private key not valid hex");
    assert!(public_hex.chars().all(|c| c.is_ascii_hexdigit()), "Public key not valid hex");
}
...
fn test_cli_generate_key_formats() {
    ...
    assert!(hex_stdout.lines().any(|line| line.len() == 64 && line.chars().all(|c| c.is_ascii_hexdigit())));

    // Test base64 format
    let base64_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--format", "base64"])
        .output()
        .expect("Failed to execute CLI");
    assert!(base64_output.status.success());
    let base64_stdout = String::from_utf8(base64_output.stdout).unwrap();
    assert!(base64_stdout.lines().any(|line| base64::decode(line).is_ok()));

    // Test PEM format
    let pem_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--format", "pem"])
        .output()
        .expect("Failed to execute CLI");
...
fn test_cli_generate_key_selective_output() {
    ...
    let public_only_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--public-only"])
        .output()
        .expect("Failed to execute CLI");
    ...
    let hex_lines: Vec<&str> = public_stdout.lines()
    ...
    let private_only_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--private-only"])
        .output()
        .expect("Failed to execute CLI");
    ...
    let hex_lines: Vec<&str> = private_stdout.lines()
        .filter(|line| line.len() == 64 && line.chars().all(|c| c.is_ascii_hexdigit()))
        .collect();
...
fn test_cli_generate_key_batch() {
    let batch_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--count", "3"])
        .output()
        .expect("Failed to execute CLI");
    ...
    let hex_lines: Vec<&str> = batch_stdout.lines()
        .filter(|line| line.len() == 64 && line.chars().all(|c| c.is_ascii_hexdigit()))
        .collect();
...
fn test_cli_generate_key_to_files() {
    ...
    let file_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "generate-key",
            "--private-key-file", private_file.to_str().unwrap(),
            "--public-key-file", public_file.to_str().unwrap()
        ])
        .output()
        .expect("Failed to execute CLI");
    
    assert!(file_output.status.success(), "CLI command failed: {}", String::from_utf8_lossy(&file_output.stderr));
    ...
    let private_content = fs::read_to_string(&private_file).expect("Failed to read private key file");
    let public_content = fs::read_to_string(&public_file).expect("Failed to read public key file");
    
    assert_eq!(private_content.trim().len(), 64, "Private key file has wrong length");
    assert_eq!(public_content.trim().len(), 64, "Public key file has wrong length");
    assert!(private_content.trim().chars().all(|c| c.is_ascii_hexdigit()), "Private key file not hex");
    assert!(public_content.trim().chars().all(|c| c.is_ascii_hexdigit()), "Public key file not hex");
}
...
fn test_cli_extract_public_key() {
    ...
    let extract_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "extract-public-key",
            "--private-key", &private_hex
        ])
        .output()
        .expect("Failed to execute CLI");
    
    assert!(extract_output.status.success(), "CLI command failed: {}", String::from_utf8_lossy(&extract_output.stderr));
    
    let extracted_public = String::from_utf8(extract_output.stdout).unwrap().trim().to_string();
    assert_eq!(extracted_public, expected_public_hex, "Extracted public key doesn't match expected");
}
...
fn test_cli_verify_key() {
    ...
    let verify_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "verify-key",
            "--private-key", &private_hex,
    ...
    assert!(verify_output.status.success(), "CLI command failed: {}", String::from_utf8_lossy(&verify_output.stderr));
    
    let verify_text = String::from_utf8(verify_output.stdout).unwrap();
    assert!(verify_text.contains("✅"), "Verification should succeed");
    assert!(verify_text.contains("verification successful"), "Should indicate successful verification");
}
...
fn test_cli_verify_key_mismatch() {
    ...
    let verify_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "verify-key",
            "--private-key", &private_hex,
            "--public-key", &wrong_public_hex
        ])
        .output()
        .expect("Failed to execute CLI");
    
    assert!(!verify_output.status.success(), "Verification should fail with mismatched keys");
    
    let verify_text = String::from_utf8(verify_output.stderr).unwrap();
    assert!(verify_text.contains("verification failed"), "Should indicate failed verification");
}
...
fn test_cli_error_handling() {
    ...
    let invalid_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "extract-public-key",
            "--private-key", "invalid_hex_key"
        ])
        .output()
        .expect("Failed to execute CLI");
    
    assert!(!invalid_output.status.success(), "Should fail with invalid private key");
    ...
    assert!(!conflict_output.status.success(), "Should fail with conflicting options");
}
...
fn test_cli_help_documentation() {
    let help_output = Command::new("cargo")
        .args(&["run", "--bin", "datafold_cli", "--", "generate-key", "--help"])
        .output()
        .expect("Failed to execute CLI");
...
fn test_cli_full_workflow() {
    ...
    let extract_output = Command::new("cargo")
        .args(&[
            "run", "--bin", "datafold_cli", "--", "extract-public-key",
            "--private-key-file", private_file.to_str().unwrap(),
            "--output-file", extracted_public_file.to_str().unwrap()
        ])
        .output()
    ...
    let extracted_public = fs::read_to_string(&extracted_public_file).unwrap().trim().to_string();
    
    assert_eq!(original_public, extracted_public, "Extracted public key should match original");
