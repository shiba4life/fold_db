...
use datafold::{
    crypto::{derive_master_keypair, generate_master_keypair, generate_salt, Argon2Params},
    db_operations::{CryptoMetadata, DbOperations},
    MasterKeyConfig,
};
...
fn test_store_and_retrieve_random_key_metadata() {
    ...
    let metadata = CryptoMetadata::new(keypair.public_key().clone(), "Random".to_string()).unwrap();
...
fn test_crypto_metadata_additional_metadata() {
    ...
    db_ops
        .update_crypto_additional_metadata("database_id".to_string(), "test_db_123".to_string())
        .unwrap();
    ...
    db_ops
        .update_crypto_additional_metadata("encryption_level".to_string(), "AES-256".to_string())
        .unwrap();
...
fn test_crypto_metadata_statistics() {
    ...
    assert_eq!(
        ...
        Some(&"Argon2id-Interactive".to_string())
    );
...
fn test_crypto_metadata_serialization_compatibility() {
    ...
    db_ops
        .update_crypto_additional_metadata("created_by".to_string(), "DataFold v0.1.0".to_string())
        .unwrap();
    ...
    assert_eq!(
        retrieved.additional_metadata.get("created_by"),
        Some(&"DataFold v0.1.0".to_string())
    );
...
fn test_crypto_metadata_error_handling() {
    ...
    let result = db_ops.update_crypto_additional_metadata("key".to_string(), "value".to_string());
...
fn test_integration_with_crypto_config() {
    ...
    for (method_name, master_key_config) in test_cases {
        ...
        let metadata =
            CryptoMetadata::new(keypair.public_key().clone(), method_name.to_string()).unwrap();
