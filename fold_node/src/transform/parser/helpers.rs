use super::grammar::{Rule, TransformParser};
...
impl TransformParser {
    ...
    pub(super) fn build_transform_decl(
        &self,
        pair: Pair<Rule>,
    ) -> Result<TransformDeclaration, SchemaError> {
        match pair.as_rule() {
                    ...
                    match pair.as_rule() {
                        ...
                        _ => {
                            return Err(SchemaError::InvalidField(format!(
                                "Unexpected rule in transform declaration: {:?}",
                                pair.as_rule()
                            )))
                        }
                    }
    ...
    pub(super) fn parse_signature_decl(&self, pair: Pair<Rule>) -> Result<String, SchemaError> {
        ...
        match sig_expr_pair.as_rule() {
            ...
            _ => Err(SchemaError::InvalidField(format!(
                "Invalid signature: {:?}",
                sig_expr_pair.as_rule()
            ))),
        }
    }

    /// Parses a logic declaration.
    pub(super) fn parse_logic_decl(
        &self,
        pair: Pair<Rule>,
    ) -> Result<Vec<Expression>, SchemaError> {
