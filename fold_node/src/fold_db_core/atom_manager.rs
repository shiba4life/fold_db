...
impl AtomManager {
    pub fn new(db_ops: DbOperations) -> Self {
        ...
        for result in db_ops.db().iter().flatten() {
            ...
            if key_str.starts_with("atom:") {
                ...
                } else if let Ok(collection) = serde_json::from_slice::<AtomRefCollection>(result.1.as_ref()) {
    ...
    pub fn get_latest_atom(&self, aref_uuid: &str) -> Result<Atom, Box<dyn std::error::Error>> {
        ...
        // Try from disk
        let aref = self.db_ops.get_item::<AtomRef>(aref_uuid)?
            .ok_or("AtomRef not found")?;

        let atom = self.db_ops.get_item::<Atom>(aref.get_atom_uuid())?
    ...
    pub fn get_atom_history(&self, aref_uuid: &str) -> Result<Vec<Atom>, Box<dyn std::error::Error>> {
        ...
        while let Some(prev_uuid) = current_atom.prev_atom_uuid() {
            current_atom = self.db_ops.get_item::<Atom>(prev_uuid)?
    ...
    pub fn update_atom_ref(
        ...
        self.ref_atoms.lock().unwrap().insert(aref_uuid.to_string(), aref.clone());
        Ok(aref)
    }
    ...
    pub fn update_atom_ref_collection(
        ...
        self.ref_collections.lock().unwrap().insert(aref_uuid.to_string(), collection.clone());
        Ok(collection)
    }
