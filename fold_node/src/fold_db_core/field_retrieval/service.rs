...
use super::{CollectionFieldRetriever, FieldRetriever, RangeFieldRetriever, SingleFieldRetriever};
...
impl FieldRetrievalService {
    ...
    pub fn get_field_value(
        ...
        info!(
            "🔍 FieldRetrievalService::get_field_value - schema: {}, field: {}",
            schema.name, field
        );
    ...
    pub fn get_field_value_with_filter(
        ...
        info!(
            "🔄 FieldRetrievalService::get_field_value_with_filter - schema: {}, field: {}",
            schema.name, field
        );
        ...
        let result = match field_def {
            ...
            FieldVariant::Collection(_) => {
                let retriever = CollectionFieldRetriever::new(atom_manager);
                info!(
                    "⚠️ Collection field does not support filtering yet, returning regular value"
                );
                FieldRetriever::get_value(&retriever, schema, field)?
            }
        };
