...
impl<'a> CollectionFieldRetriever<'a> {
    ...
    fn load_atom_ref_collection(&self, ref_atom_uuid: &str) -> Result<Option<crate::atom::AtomRefCollection>, SchemaError> {
        match self.base.atom_manager.get_ref_collections().lock() {
            Ok(collections_guard) => {
                Ok(collections_guard.get(ref_atom_uuid).cloned())
            }
            Err(e) => {
                info!("❌ Failed to acquire ref_collections lock: {:?}", e);
                Err(SchemaError::InvalidData("Failed to access collection data".to_string()))
            }
        }
    }

    /// Converts AtomRefCollection to JSON format by loading actual atom content
    fn convert_collection_to_json(&self, _atom_ref_collection: &crate::atom::AtomRefCollection) -> Result<Value, SchemaError> {
        ...
        self.base.retrieve_field_value(
            ...
            |ref_atom_uuid| {
                match self.load_atom_ref_collection(ref_atom_uuid)? {
                    ...
                    None => {
                        info!("⚠️  No AtomRefCollection found with UUID: {}", ref_atom_uuid);
                        Ok(Value::Null)
                    }
                }
            },
