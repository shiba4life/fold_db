    ...
    pub fn register_transform(
        ...
        if let Some(db_ops) = &self.db_ops {
            ...
            return Err(SchemaError::InvalidData(
                "DbOperations not available".to_string(),
            ));
        }
        ...
        // Register the output atom reference
        {
            ...
            info!(
                "Registering transform output: {} -> {}",
                transform_id, output_aref
            );
            transform_outputs.insert(transform_id.clone(), output_aref.clone());
            info!(
                "Transform outputs now contains {} entries",
                transform_outputs.len()
            );
        }
    ...
    pub fn unregister_transform(&self, transform_id: &str) -> Result<bool, SchemaError> {
        ...
        let found = if let Some(db_ops) = &self.db_ops {
            ...
            match db_ops.delete_transform(transform_id) {
                Ok(existed) => {
                    if existed {
                        let mut registered_transforms =
                            self.registered_transforms.write().map_err(|_| {
