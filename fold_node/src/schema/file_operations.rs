...
impl SchemaFileOperations {
    ...
    pub fn write_schema_to_file(json_schema: &JsonSchemaDefinition, final_name: &str, directory: &str) -> Result<(), SchemaError> {
        ...
        if let Some(parent) = target_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| SchemaError::InvalidData(format!("Failed to create directory: {}", e)))?;
        }
        ...
        let hash = super::hasher::SchemaHasher::add_hash_to_schema(&mut schema_with_hash)
            .map_err(|e| SchemaError::InvalidData(format!("Failed to add hash to schema: {}", e)))?;
        ...
        info!("Schema '{}' successfully written to {}", final_name, target_path.display());
        Ok(())
    }
    ...
    pub fn read_schema_file(file_path: &Path) -> Result<Value, SchemaError> {
        let content = std::fs::read_to_string(file_path)
            .map_err(|e| SchemaError::InvalidData(format!("Failed to read file {}: {}", file_path.display(), e)))?;
        
        serde_json::from_str(&content)
            .map_err(|e| SchemaError::InvalidData(format!("Failed to parse JSON from {}: {}", file_path.display(), e)))
    }
