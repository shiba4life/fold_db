use crate::atom::{Atom, AtomRef, AtomRefBehavior, AtomStatus};
...
mod tests {
    ...
    fn test_execute_transform() {
        ...
        assert_eq!(result_value, &json!(15), "Unexpected transform result");
        ...
        assert_eq!(output_atom.content(), &json!(15), "Output atom not updated correctly");
    }
    ...
    fn test_transform_chain() {
        ...
        // Register the first transform
        let result = registry.register_transform(
            "test_transform1".to_string(),
            transform1,
            vec!["input1".to_string(), "input2".to_string()],
            "output1".to_string(),
        );
        
        assert!(result.is_ok(), "Failed to register first transform");
        
        // Create a dummy output1 atom reference
        let output1_aref = AtomRef::new("dummy".to_string(), "test_key".to_string());
        store.atom_refs.lock().unwrap().insert("output1".to_string(), output1_aref);
        
        // Create second transform that depends on the output of the first
        ...
        // Create a dummy output2 atom reference
        let output2_aref = AtomRef::new("dummy".to_string(), "test_key".to_string());
        store.atom_refs.lock().unwrap().insert("output2".to_string(), output2_aref);
        
        // Execute the first transform
        let results = registry.handle_atom_ref_update("input1");
        
        assert_eq!(results.len(), 1, "Expected one transform result");
        assert!(results[0].is_ok(), "First transform execution failed");
        
        // Execute the second transform
        let results = registry.handle_atom_ref_update("output1");
        
        assert_eq!(results.len(), 1, "Expected one transform result");
        assert!(results[0].is_ok(), "Second transform execution failed");
        
        // Check the final result
        let atom_refs = store.atom_refs.lock().unwrap();
        let output2_aref = atom_refs.get("output2").unwrap();
        
        let atoms = store.atoms.lock().unwrap();
        let output2_atom = atoms.get(output2_aref.get_atom_uuid()).unwrap();
        
        assert_eq!(output2_atom.content(), &json!(30), "Transform chain result incorrect");
    }
    ...
    fn test_unregister_transform() {
        ...
        let transform_inputs = registry.get_transform_inputs("test_transform");
        assert!(transform_inputs.is_empty(), "Transform inputs not unregistered correctly");
        
        let transform_output = registry.get_transform_output("test_transform");
        assert_eq!(transform_output, None, "Transform output not unregistered correctly");
    }
}