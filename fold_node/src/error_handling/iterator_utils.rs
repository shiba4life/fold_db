...
impl IteratorUtils {
        ...
        I: Iterator<Item = T>,
    {
        ...
        for (i, item) in iter.enumerate() {
            if i >= limit {
                return Err(SchemaError::InvalidData(format!("Iterator exceeded limit {} in {}", limit, context)));
            }
            result.push(item);
        }
        Ok(result)
...
mod tests {
    ...
    fn test_next_with_context() {
        ...
        assert_eq!(IteratorUtils::next_with_context(&mut iter, "test").unwrap(), 1);
        assert_eq!(IteratorUtils::next_with_context(&mut iter, "test").unwrap(), 2);
        assert_eq!(IteratorUtils::next_with_context(&mut iter, "test").unwrap(), 3);
