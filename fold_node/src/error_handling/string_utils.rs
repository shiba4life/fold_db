...
impl StringUtils {
    ...
    pub fn safe_substring<'a>(s: &'a str, start: usize, end: usize, context: &str) -> Result<&'a str, SchemaError> {
        if start > s.len() || end > s.len() || start > end {
            return Err(SchemaError::InvalidData(format!(
                "Invalid substring bounds [{}, {}] for string of length {} in {}", 
                start, end, s.len(), context
            )));
        }
        Ok(&s[start..end])
    }
    ...
    {
        s.parse()
            .map_err(|e| SchemaError::InvalidData(format!("Failed to parse '{}' as number in {}: {}", s, context, e)))
    }
    
    /// Safely split string and get specific part
    pub fn split_and_get<'a>(s: &'a str, delimiter: char, index: usize, context: &str) -> Result<&'a str, SchemaError> {
        s.split(delimiter)
            .nth(index)
    ...
    pub fn trim_non_empty<'a>(s: &'a str, context: &str) -> Result<&'a str, SchemaError> {
        let trimmed = s.trim();
        if trimmed.is_empty() {
            Err(SchemaError::InvalidData(format!("String is empty after trimming in {}", context)))
        } else {
            Ok(trimmed)
        }
    }
...
mod tests {
    ...
    fn test_safe_substring() {
        let s = "hello world";
        assert_eq!(StringUtils::safe_substring(s, 0, 5, "test").unwrap(), "hello");
        assert_eq!(StringUtils::safe_substring(s, 6, 11, "test").unwrap(), "world");
    ...
    fn test_trim_non_empty() {
        assert_eq!(StringUtils::trim_non_empty("  hello  ", "test").unwrap(), "hello");
