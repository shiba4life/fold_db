...
use crate::ingestion::{IngestionConfig, IngestionError, IngestionResult, IngestionResponse};
use crate::ingestion::core::IngestionRequest;
use crate::ingestion::openrouter_service::{OpenRouterService, AISchemaResponse};
...
impl SimpleIngestionService {
    ...
    pub async fn process_json_with_node(
        ...
        info!("Retrieved {} available schemas", available_schemas.as_object().map(|o| o.len()).unwrap_or(0));

        // Step 3: Get AI recommendation
        let ai_response = self.openrouter_service
    ...
    fn get_stripped_available_schemas_from_node(&self, node: &DataFoldNode) -> IngestionResult<Value> {
        // Get all available schemas from the node
        let schema_states = node.list_schemas_with_state()
            .map_err(|e| IngestionError::SchemaSystemError(
    ...
    fn determine_schema_to_use(&self, ai_response: &AISchemaResponse, node: &mut DataFoldNode) -> IngestionResult<String> {
    ...
    fn create_new_schema_with_node(&self, schema_def: &Value, node: &mut DataFoldNode) -> IngestionResult<String> {
    ...
    fn parse_schema_definition(&self, schema_def: &Value) -> IngestionResult<crate::schema::Schema> {
        info!("Parsing schema definition: {}", serde_json::to_string_pretty(schema_def).unwrap_or_else(|_| "Invalid JSON".to_string()));
        ...
        if let Some(obj) = schema_def.as_object() {
            if obj.len() == 1 {
                ...
                if let Ok(schema) = serde_json::from_value::<crate::schema::Schema>(schema_content.clone()) {
                    info!("Successfully parsed wrapped schema");
                    return Ok(schema);
                }
                
                // If that fails, try to create a basic schema from the wrapped content
                return self.create_basic_schema_from_wrapped_definition(schema_name, schema_content);
            }
        }
    ...
    fn create_basic_schema_from_wrapped_definition(&self, schema_name: &str, schema_content: &Value) -> IngestionResult<crate::schema::Schema> {
        info!("Creating basic schema from wrapped definition for: {}", schema_name);
        ...
        if let Some(Value::Object(fields)) = obj.get("fields") {
            info!("Processing {} fields for schema {}", fields.len(), schema_name);
        ...
        info!("Successfully created schema '{}' with {} fields", schema_name, schema.fields.len());
        Ok(schema)
    }

    /// Create a basic schema from a simple definition
    fn create_basic_schema_from_definition(&self, schema_def: &Value) -> IngestionResult<crate::schema::Schema> {
    ...
    fn execute_mutations_with_node(&self, mutations: &[Mutation], node: &mut DataFoldNode) -> IngestionResult<usize> {
