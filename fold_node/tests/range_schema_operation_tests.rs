...
fn create_comprehensive_range_schema() -> Schema {
    ...
    schema.add_field("achievements".to_string(), FieldVariant::Range(range_field3));
...
fn create_range_mutation_single_user(user_id: &str) -> Mutation {
    ...
    fields.insert("game_scores".to_string(), json!({
        user_id: {
            "tetris": 85000,
            "pacman": 12500,
            "chess": 1200
    ...
    fields.insert("achievements".to_string(), json!({
        user_id: {
    ...
    fields.insert("statistics".to_string(), json!({
        user_id: {
            "total_games": 450,
            "win_rate": 0.75,
...
fn test_range_schema_create_single_user_success() {
    ...
    assert!(result.is_ok(), "Range schema mutation should succeed: {:?}", result);
}
...
fn test_range_schema_create_multiple_users() {
    ...
    for user_id in &users {
        ...
        let result = fold_db.write_schema(mutation);
        assert!(result.is_ok(), "Range schema mutation for {} should succeed: {:?}", user_id, result);
    }
}
...
fn test_range_schema_update_existing_user() {
    ...
    assert!(result.is_ok(), "Range schema update mutation should succeed: {:?}", result);
}
...
fn test_range_schema_different_data_types() {
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_ok(), "Range schema mutation with mixed data types should succeed: {:?}", result);
}
...
fn test_range_schema_large_dataset_mutation() {
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_ok(), "Range schema mutation with large dataset should succeed: {:?}", result);
}
...
fn test_range_schema_query_specific_user() {
    ...
    assert!(grouped_obj.contains_key("user_456"), "Should contain results for user_456");
    assert!(!grouped_obj.contains_key("user_123"), "Should not contain results for user_123");
    assert!(!grouped_obj.contains_key("user_789"), "Should not contain results for user_789");
}
...
fn test_range_schema_query_multiple_fields() {
    ...
    let query = Query::new_with_filter(
        "UserScores".to_string(),
        vec!["game_scores".to_string(), "achievements".to_string(), "statistics".to_string()],
        "test_pubkey".to_string(),
        0,
        Some(json!({
            "range_filter": {
    ...
    let result = &results[0];
    assert!(result.is_ok(), "Multi-field range schema query should succeed: {:?}", result);
    ...
    assert!(user_data.contains_key("game_scores"), "Should contain game_scores");
    assert!(user_data.contains_key("achievements"), "Should contain achievements");
    assert!(user_data.contains_key("statistics"), "Should contain statistics");
}
...
fn test_range_schema_query_single_field() {
    ...
    let result = &results[0];
    assert!(result.is_ok(), "Single field range schema query should succeed: {:?}", result);
    
    let grouped_result = result.as_ref().unwrap();
    let grouped_obj = grouped_result.as_object().unwrap();
    let user_data = grouped_obj.get("single_field_user").unwrap().as_object().unwrap();
    
    // Verify only requested field is present
    assert!(user_data.contains_key("achievements"), "Should contain achievements");
    assert!(!user_data.contains_key("game_scores"), "Should not contain game_scores");
    assert!(!user_data.contains_key("statistics"), "Should not contain statistics");
}
...
fn test_range_schema_query_nonexistent_user() {
    ...
    let result = &results[0];
    assert!(result.is_ok(), "Query for nonexistent user should succeed but return empty: {:?}", result);
    
    let grouped_result = result.as_ref().unwrap();
    let grouped_obj = grouped_result.as_object().unwrap();
    assert!(grouped_obj.is_empty() || !grouped_obj.contains_key("nonexistent_user"), 
...
fn test_range_schema_query_after_update() {
    ...
    let result = &results[0];
    assert!(result.is_ok(), "Query after update should succeed: {:?}", result);
    
    let grouped_result = result.as_ref().unwrap();
    let grouped_obj = grouped_result.as_object().unwrap();
    let user_data = grouped_obj.get("update_test_user").unwrap().as_object().unwrap();
    
    // Verify updated data is present
    let game_scores = user_data.get("game_scores").unwrap().as_object().unwrap();
    assert!(game_scores.contains_key("tetris"), "Should contain updated tetris score");
    assert!(game_scores.contains_key("snake"), "Should contain new snake score");
    
    let achievements = user_data.get("achievements").unwrap().as_object().unwrap();
    assert!(achievements.contains_key("perfectionist"), "Should contain new achievement");
}
...
fn test_range_schema_query_missing_range_filter() {
    ...
    let result = &results[0];
    assert!(result.is_ok(), "Should fall back to field query: {:?}", result);
}
...
fn test_range_schema_query_invalid_range_filter() {
    ...
    let result = &results[0];
    assert!(result.is_err(), "Query with invalid range_filter should fail");
...
fn test_range_schema_mutation_empty_string_range_key() {
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_err(), "Mutation with empty string range_key should fail");
...
fn test_range_schema_concurrent_mutations() {
    ...
    for i in 0..10 {
        ...
        let result = results[0].as_ref().unwrap();
        let grouped_obj = result.as_object().unwrap();
        assert!(grouped_obj.contains_key(&user_id), "Should contain data for {}", user_id);
    }
}
...
fn test_range_schema_stress_test_large_mutations() {
    ...
    for i in 0..1000 {
        large_game_scores.insert(format!("game_{:04}", i), json!({
            "score": i * 100,
            "level": i / 10,
    ...
    let result = fold_db.write_schema(mutation);
    assert!(result.is_ok(), "Large mutation should succeed: {:?}", result);
    ...
    let result = results[0].as_ref().unwrap();
    let grouped_obj = result.as_object().unwrap();
    let user_data = grouped_obj.get("stress_test_user").unwrap().as_object().unwrap();
    let game_scores = user_data.get("game_scores").unwrap().as_object().unwrap();
    
    assert_eq!(game_scores.len(), 1000, "Should contain all 1000 game scores");
}