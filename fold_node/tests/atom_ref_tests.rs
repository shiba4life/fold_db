use chrono::Utc;
use fold_node::atom::{Atom, AtomRef, AtomRefBehavior, AtomRefCollection, AtomRefRange};
use serde_json::json;
...
fn test_atom_ref_range() {
    ...
    for (key, atom) in ["a", "b", "c"].iter().zip(&atoms) {
        assert_eq!(range.get_atom_uuid(key), Some(&atom.uuid().to_string()));
    }

    assert_eq!(range.get_atom_uuid("b"), Some(&atoms[1].uuid().to_string()));
    assert_eq!(range.remove_atom_uuid("b"), Some(atoms[1].uuid().to_string()));
