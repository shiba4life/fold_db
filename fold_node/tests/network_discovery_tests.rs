...
async fn test_discovery_initialization() {
    ...
    assert!(
        result.is_ok(),
        "Failed to initialize network with discovery enabled: {:?}",
        result
    );
    ...
    assert!(
        result.is_ok(),
        "Failed to start network with discovery enabled: {:?}",
        result
    );
...
async fn test_node_discovery_enabled() {
    ...
    assert!(
        known_nodes1.is_ok(),
        "Failed to get known nodes from node 1: {:?}",
        known_nodes1
    );
    ...
    assert!(
        known_nodes2.is_ok(),
        "Failed to get known nodes from node 2: {:?}",
        known_nodes2
    );
    ...
    println!(
        "Node 1 known nodes: {:?}",
        known_nodes1.keys().collect::<Vec<_>>()
    );
    println!(
        "Node 2 known nodes: {:?}",
        known_nodes2.keys().collect::<Vec<_>>()
    );
...
async fn test_node_discovery_disabled() {
    ...
    let discovered_nodes1 = node1.discover_nodes().await;
    assert!(
        discovered_nodes1.is_ok(),
        "Failed to discover nodes from node 1: {:?}",
        discovered_nodes1
    );
    ...
    assert!(
        discovered_nodes.is_empty(),
        "Expected empty list with discovery disabled, got: {:?}",
        discovered_nodes
    );
...
async fn test_manual_node_connection() {
    ...
    assert!(
        trusted_nodes.contains_key(&node2_id),
        "Node 2 not found in trusted nodes"
    );
...
async fn test_discovery_with_multiple_nodes() {
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        assert!(
            known_nodes.is_ok(),
            "Failed to get known nodes from node {}: {:?}",
            i,
            known_nodes
        );
        ...
        println!(
            "Node {} known nodes: {:?}",
            i,
            known_nodes.keys().collect::<Vec<_>>()
        );
