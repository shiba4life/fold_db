...
fn test_schema_aref_preservation_on_reload() {
    ...
    manager
        .add_schema_available(original_schema.clone())
        .unwrap();
    ...
    assert!(
        reloaded_schema.is_none(),
        "Schema should remain unloaded after reload from disk"
    );
    ...
    manager
        ...
        .unwrap();
    manager.approve_schema("aref_preservation_test").unwrap();
    let loaded_schema = manager
        .get_schema("aref_preservation_test")
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        &original_ref_uuid, reloaded_ref_uuid,
...
fn test_schema_state_preservation_on_reload() {
    ...
    let reloaded_schema = new_manager
        .get_schema("state_preservation_test")
        .unwrap()
...
fn test_folddb_aref_preservation_on_reload() {
    ...
    assert_eq!(
        &original_ref_uuid, loaded_ref_uuid,
...
fn test_schema_without_ref_atom_uuid_gets_new_one() {
    ...
    use fold_node::testing::{
        FieldPaymentConfig, FieldVariant, PermissionsPolicy, SingleField, TrustDistanceScaling,
    };
    ...
    assert!(
        assigned_ref_uuid.is_some(),
        "Field should have been assigned a ref_atom_uuid"
    );
    assert!(
        !assigned_ref_uuid.unwrap().is_empty(),
        "ref_atom_uuid should not be empty"
    );
}
