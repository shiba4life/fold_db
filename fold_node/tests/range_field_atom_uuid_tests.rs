...
use fold_node::atom::{AtomRefRange, AtomRefBehavior};
...
fn test_atom_ref_range_missing_uuid_deserialization() {
    ...
    assert!(result.is_err(), "Expected deserialization to fail due to missing uuid field");
    
    let error_msg = result.unwrap_err().to_string();
    println!("Deserialization error: {}", error_msg);
    assert!(error_msg.contains("missing field"), "Error should mention missing field");
}
...
fn test_atom_ref_range_complete_serialization() {
    ...
    atom_ref_range.set_atom_uuid("2024-01-01:hourly:00".to_string(), "atom_uuid_2".to_string());
    ...
    assert!(serialized_value.get("uuid").is_some(), "uuid field should be present");
    assert!(serialized_value.get("atom_uuids").is_some(), "atom_uuids field should be present");
    assert!(serialized_value.get("updated_at").is_some(), "updated_at field should be present");
    assert!(serialized_value.get("status").is_some(), "status field should be present");
    assert!(serialized_value.get("update_history").is_some(), "update_history field should be present");
    
    // Test deserialization
    let deserialized: AtomRefRange = serde_json::from_str(&serialized).unwrap();
    
    // Verify the data is preserved
    assert_eq!(deserialized.get_atom_uuid("2024-01-01:daily"), Some(&"atom_uuid_1".to_string()));
    assert_eq!(deserialized.get_atom_uuid("2024-01-01:hourly:00"), Some(&"atom_uuid_2".to_string()));
    assert_eq!(deserialized.get_atom_uuid("2024-01-02:daily"), Some(&"atom_uuid_3".to_string()));
...
fn test_atom_ref_range_serialized_structure() {
    ...
    println!("Complete AtomRefRange structure: {}", serde_json::to_string_pretty(&serialized_value).unwrap());
}
...
fn test_range_field_with_proper_atom_ref_range() {
    ...
    println!("Serialized RangeField: {}", serde_json::to_string_pretty(&serialized).unwrap());
    ...
    if let Some(atom_ref_range) = deserialized.atom_ref_range() {
        assert_eq!(atom_ref_range.get_atom_uuid("2024-01-01:daily"), Some(&"atom_uuid_1".to_string()));
        assert_eq!(atom_ref_range.get_atom_uuid("2024-01-01:hourly:00"), Some(&"atom_uuid_2".to_string()));
    }
}
...
fn test_field_manager_range_field_save_fetch_cycle() {
    ...
    schema.fields.insert("metrics_by_timeframe".to_string(), FieldVariant::Range(range_field));
    ...
    let set_result = field_manager.set_field_value(&mut schema, "metrics_by_timeframe", test_data.clone(), source_pub_key);
    ...
    println!("Get field result WITHOUT ref_atom_uuid: {:?}", get_result_without_ref);
    assert_eq!(get_result_without_ref.unwrap(), Value::Null, "Should return Null when ref_atom_uuid is not set");

    // Update the schema to include the ref_atom_uuid (simulating what the mutation logic does)
    if let Some(FieldVariant::Range(ref mut range_field)) = schema.fields.get_mut("metrics_by_timeframe") {
        println!("Setting ref_atom_uuid to: {}", atom_uuid);
        range_field.inner.ref_atom_uuid = Some(atom_uuid.clone());
        println!("ref_atom_uuid after setting: {:?}", range_field.inner.ref_atom_uuid);
    }
    ...
    if let Some(FieldVariant::Range(range_field)) = schema.fields.get("metrics_by_timeframe") {
        println!("Verified ref_atom_uuid: {:?}", range_field.inner.ref_atom_uuid);
    }
    ...
    assert_eq!(retrieved_data, test_data, "Retrieved data should match original data after fix");
...
fn test_range_field_with_atom_ref_range_persistence() {
    ...
    assert!(serialized.get("atom_ref_range").is_some(), "atom_ref_range should be serialized");
    ...
    if let Some(atom_ref_range) = deserialized.atom_ref_range() {
        assert_eq!(atom_ref_range.get_atom_uuid("2024-01-01:daily"), Some(&"atom_uuid_1".to_string()));
        assert_eq!(atom_ref_range.get_atom_uuid("2024-01-01:hourly:00"), Some(&"atom_uuid_2".to_string()));
        assert_eq!(atom_ref_range.get_atom_uuid("2024-01-02:daily"), Some(&"atom_uuid_3".to_string()));
    }
}
...
fn test_event_analytics_range_field_scenario() {
    ...
    match field_def {
        FieldVariant::Range(range_field) => {
            ...
            let serialized = serde_json::to_value(range_field).unwrap();
            println!("EventAnalytics range field serialized: {}", serde_json::to_string_pretty(&serialized).unwrap());
            
            // Verify atom_ref_range structure
            if let Some(atom_ref_range_value) = serialized.get("atom_ref_range") {
                assert!(atom_ref_range_value.get("uuid").is_some(), "atom_ref_range should have uuid");
                assert!(atom_ref_range_value.get("atom_uuids").is_some(), "atom_ref_range should have atom_uuids");
                assert!(atom_ref_range_value.get("updated_at").is_some(), "atom_ref_range should have updated_at");
                assert!(atom_ref_range_value.get("status").is_some(), "atom_ref_range should have status");
                assert!(atom_ref_range_value.get("update_history").is_some(), "atom_ref_range should have update_history");
            }
        }
