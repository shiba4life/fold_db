...
use fold_node::schema::types::{Transform, SchemaError};
...
fn test_transform_manager_integration_with_unified_operations() {
    ...
    assert!(transforms.contains(&"test_transform".to_string()), "Transform should be in list");
    ...
    assert!(mapping_result.is_ok(), "Should be able to store transform mapping");

    let retrieved_mapping = db_ops.get_transform_mapping("test_mapping").unwrap();
    assert!(retrieved_mapping.is_some(), "Transform mapping should be retrievable");
...
fn test_transform_manager_constructor_with_unified_operations() {
    ...
    db_ops.store_transform("preloaded_transform", &transform).unwrap();

    // Create minimal callback functions
    let get_atom_fn = Arc::new(|_aref_uuid: &str| -> Result<Atom, Box<dyn std::error::Error>> {
        let atom = Atom::new("test_schema".to_string(), "test_pub_key".to_string(), serde_json::json!({"value": 1}));
        Ok(atom)
    });

    let create_atom_fn = Arc::new(|schema_name: &str,
                                  source_pub_key: String,
                                  _prev_atom_uuid: Option<String>,
                                  content: Value,
                                  _status: Option<fold_node::atom::AtomStatus>| -> Result<Atom, Box<dyn std::error::Error>> {
        let atom = Atom::new(schema_name.to_string(), source_pub_key, content);
        Ok(atom)
    });

    let update_atom_ref_fn = Arc::new(|aref_uuid: &str,
                                      atom_uuid: String,
                                      _source_pub_key: String| -> Result<AtomRef, Box<dyn std::error::Error>> {
        let atom_ref = AtomRef::new(aref_uuid.to_string(), atom_uuid);
        Ok(atom_ref)
    });

    let get_field_fn = Arc::new(|_schema_name: &str, _field_name: &str| -> Result<Value, SchemaError> {
    ...
    assert!(transform_manager_result.is_ok(), "TransformManager should be created successfully with unified operations");
    
    let transform_manager = transform_manager_result.unwrap();

    // Verify the preloaded transform exists
    let exists = transform_manager.transform_exists("preloaded_transform").unwrap();
    assert!(exists, "Preloaded transform should exist in TransformManager");
