...
use datafold::config::{
    EnhancedConfigurationManager, EnhancedConfig, ConfigMigrationManager, MigrationStrategy,
    create_platform_keystore, get_platform_info, EnhancedPlatformInfo,
    ConfigChangeEvent, ConfigChangeType, ConfigChangeSource, ConfigValue,
};
...
async fn demo_platform_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  - Keystore Available: {}", enhanced_info.keystore_available);
    println!("  - File Watching Available: {}", enhanced_info.file_watching_available);
    println!("  - Atomic Operations Available: {}", enhanced_info.atomic_operations_available);
    println!("  - Memory Mapping Available: {}", enhanced_info.memory_mapping_available);
...
async fn demo_enhanced_config_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Platform optimizations: {}", config.platform_settings.enable_optimizations);
    println!("   Lazy loading: {}", config.performance.lazy_loading);
    println!("   Auto-encrypt sensitive: {}", config.security_enhanced.auto_encrypt_sensitive);
    ...
    app_config.insert("name".to_string(), ConfigValue::string("DataFold Platform Demo"));
    app_config.insert("version".to_string(), ConfigValue::string("2.0.0"));
    app_config.insert("environment".to_string(), ConfigValue::string("development"));
    enhanced_config.base.set_section("app".to_string(), ConfigValue::object(app_config));
    ...
    enhanced_config.base.set_section("database".to_string(), ConfigValue::object(db_config));
...
async fn demo_keystore_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if keystore.is_available() {
        ...
        keystore.store_secret("demo_sensitive_config", sensitive_data.as_bytes()).await?;
        ...
        if let Some(retrieved_data) = keystore.get_secret("demo_sensitive_config").await? {
            ...
            println!("   API Key: {}", config["api_key"].as_str().unwrap_or("N/A"));
...
async fn demo_configuration_migration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Sections migrated: {}", migration_result.sections_migrated);
...
async fn demo_change_notifications() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.on_change(move |event: ConfigChangeEvent| {
    ...
    new_config.base.set_section("features".to_string(), ConfigValue::object(new_section));
...
fn display_config_tree(config: &EnhancedConfig, indent: usize) {
    ...
    println!("{}│   ├── Optimizations: {}", prefix, config.platform_settings.enable_optimizations);
    println!("{}│   ├── Native File Ops: {}", prefix, config.platform_settings.use_native_file_ops);
    println!("{}│   └── Memory Mapping: {}", prefix, config.platform_settings.enable_memory_mapping);
    println!("{}├── Performance Settings:", prefix);
    println!("{}│   ├── Lazy Loading: {}", prefix, config.performance.lazy_loading);
    println!("{}│   ├── Cache TTL: {}s", prefix, config.performance.cache_ttl_secs);
    println!("{}│   └── Max Cache Size: {}MB", prefix, config.performance.max_cache_size_mb);
    println!("{}└── Sections: {} configured", prefix, config.base.sections.len());
}
...
fn create_sample_config() -> EnhancedConfig {
    ...
    config.base.set_section("app".to_string(), ConfigValue::object(app_config));
    ...
    config.base.set_section("service".to_string(), ConfigValue::object(service_config));
